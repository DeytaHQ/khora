...
- **Embedded VectorCypher multi-hop graph traversal now crosses the second hop** (#1086): on the embedded SQLite + LanceDB stack, a `recall(..., graph_depth=2)` did not surface a 2-hop-reachable entity. Two wiring gaps: `Khora.recall()` did not accept a `graph_depth` kwarg (so passing it raised `TypeError`), and the VectorCypher retriever called the storage coordinator's `get_neighborhoods_batch` without the required `namespace_id`, which raised `TypeError` inside the graph-expansion step and crashed the recall before the recursive CTE ran. `Khora.recall()` now accepts `graph_depth` and forwards it (VectorCypher-only; other engines ignore it), and the retriever passes `namespace_id` through to the coordinator. The recursive CTE that walks edges was already correct.
