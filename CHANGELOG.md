...
- **Weaviate skeleton backend hardening: async client + auth + Weaviate Cloud + CI** (#783). `WeaviateTemporalStore` now uses `weaviate.use_async_with_local / use_async_with_custom / use_async_with_weaviate_cloud` (the v4 `WeaviateAsyncClient` family) so Skeleton engine I/O does not block the event loop. New `WeaviateBackendConfig` dataclass exposes optional `api_key` (auth for self-hosted clusters with `AUTHENTICATION_APIKEY_ENABLED=true`), `cluster_url` (Weaviate Cloud), `grpc_port` / `http_secure` / `grpc_secure` (custom self-hosted with non-default ports or TLS), and `additional_headers`. The legacy `WeaviateTemporalStore(config, "http://...")` constructor signature is preserved - the URL string wraps into a default `WeaviateBackendConfig`. CI gains a `weaviate-integration` job that spins up a Weaviate service-container side-car and runs `tests/integration/test_weaviate_async_integration.py` (gated behind `WEAVIATE_INTEGRATION_TEST=1` so it stays opt-in for local runs). 26 new unit tests cover backend-config validation, connect routing (local / custom / cloud), tenant cache, health check, and CRUD - all with a mocked async client. See `docs/engines/skeleton-engine.md` for the auth / cloud usage. Out of scope for this PR (filed separately): Chronicle / VectorCypher Weaviate support, turbopuffer evaluation.
