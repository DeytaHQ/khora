...
- **Cross-namespace IDOR family — direct sub-backend read paths** (IGR-221 / IGR-223, umbrella IGR-222). IGR-212 / IGR-213 / IGR-214 (PRs #721 / #722) hardened three coordinator methods by post-fetch namespace comparison. The vulnerability surface was actually much wider: every `StorageCoordinator.{graph,vector,relational,event_store}` sub-backend (public dataclass fields) exposed un-namespaced `get_*` / `entity_exists` / traversal methods that returned rows across tenants given just an id. The `GraphBackendProtocol` and `RelationalBackendProtocol` themselves declared read-by-id methods without a `namespace_id` argument — every backend implementation correctly mirrored the Protocol, so the bug was uniform across surrealdb / pgvector / postgresql / sqlite_lance / neo4j / age / memgraph / neptune. **Tightened the Protocol contracts** in `src/khora/storage/backends/base.py`: 13 read methods now require `*, namespace_id: UUID` (kwarg-only). Every backend implementation filters at the SQL / Cypher / SurrealQL layer — not post-fetch (post-fetch leaks timing as an existence oracle and still touches foreign rows in the DB). The `StorageCoordinator` facade and every in-tree caller (engines/vectorcypher, engines/chronicle, engines/skeleton, query/agentic, query/engine, pipelines/flows/ingest, top-level `khora.py`) thread the namespace through. **Methods tightened:** `RelationalBackend.get_document` / `get_documents_batch` / `get_document_sources_batch`; `VectorBackend.entity_exists` plus pgvector-specific `get_entity` / `get_entities_batch`; `GraphBackend.get_entity` / `get_entities_batch` / `get_relationship` / `get_episode` / `get_entity_relationships` / `get_neighborhood` / `get_neighborhoods_batch`; `EventStore.get_events_for_resource` / `get_latest_event`; plus the `SurrealDB.vector.get_entity` silent-accept bug (kwarg present, docstring admitted it was ignored, SQL didn't filter — now actually filters). Graph traversal (`get_neighborhood*`, `find_paths`) filters at every hop so a traversal seeded inside namespace A cannot visit a node in namespace B. **Breaking:** `Khora.get_document(doc_id, *, namespace=…)` now requires the namespace kwarg; same for the storage coordinator/backend methods listed. Per `docs/consumers.md` §"Security exception", this carve-out is sanctioned for patch releases; khora-cli / khora-explorer / external consumer surfaces are unaffected (none touch `kb.storage.{graph,vector,relational,event_store}` directly). Sister tickets in this v0.15.2 patch family: IGR-224 (privatize sub-backend attributes with deprecation shims), IGR-225 (property-based cross-namespace IDOR regression test), IGR-226 (write/delete tightening + AGE Cypher-injection hardening).
