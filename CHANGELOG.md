...
- **`Khora.list_documents` return type changed from `list[Document]` to `DocumentPage`.** `DocumentPage` subclasses `collections.abc.Sequence`, so iteration, `len()`, indexing, slicing and truthiness are unchanged — a caller that only iterates or indexes the result is source-compatible. What breaks: code that relied on the concrete `list` type — mutation (`.append`/`.extend`/`+=`), `list`-identity checks (`type(r) is list`), concatenation with `+ [...]`, or a type annotation of `list[Document]`. Wrap in `list(page)` at the boundary if a real list is required. (ADR-123 V1 edge case #13.)
