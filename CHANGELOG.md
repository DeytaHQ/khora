...
- **Chunk titles are now lexically searchable** (#1574, PR #1613). `khora_chunks.title` was stored and prepended to content for *embeddings* at ingest, but every full-text index covered `content` only — a recall query quoting a document's exact title (file names, sheet codes, dates: rare, exact tokens, precisely what a lexical channel exists to catch) returned 0 rows from BM25/FTS on every backend. The two temporal stores behind `enable_bm25_channel` now index it: `sqlite_lance` adds `title` as a second FTS5 column (all three sync triggers carry it, external-content delete symmetry preserved), and pgvector's `content_tsv` trigger becomes `setweight(to_tsvector(title), 'A') || setweight(to_tsvector(content), 'B')`. Verified on both tokenizers that filename-style titles (`Floor Panels_Dimensioned_20260213`) split into matchable tokens — no normalization layer was needed. Population needed no change: `document_denorm_fields()` has stamped `title` onto every chunk since 041/044.
- **New setting `query.bm25_title_weight`** (env `KHORA_QUERY_BM25_TITLE_WEIGHT`, default `1.0`, clamped `[0, 10]`) — query-time weight of a chunk's title vs its content *within* the lexical channel; the channel's own influence in fusion stays governed by `keyword_weight`, and RRF is rank-only so the title weight never leaks into fused scores directly. At the default a content token scores exactly what it scored before (PG relabels content D→B but `ts_rank_cd` always receives an explicit weights vector pinning both to 0.1; SQLite emits the bare `bm25()` call). **`1.0` is not "no change":** folding title into the index means title-matching chunks newly match and can re-rank results at every weight — that gap-closing is the point of the release. The weight participates in the recall-cache fingerprint. Accepted-but-inert on `surrealdb` / `turbopuffer` / `weaviate` temporal stores and the Chronicle legacy-chunks path (follow-up tracked upstream).
- **ADR-001 degradation for inapplicable title weights.** A non-neutral `bm25_title_weight` against an embedded database whose FTS table predates the title column falls back to content-only ranking — never an error — and records `Degradation(component="vectorcypher.bm25_title_weight", reason="fts_missing_title_column")` on `RecallResult.engine_info["degradations"]` plus the new internal counter `khora.vectorcypher.bm25_title_weight.degraded_total` (one per recall, not per channel; no `namespace_id` label). Pure-vector searches (`SearchMode.VECTOR`) never record it — no lexical scoring runs there.

### Changed

- **Migration 058 (`khora_chunks_title_fts`) — read before deploying at scale.** PG-only, reversible. Swaps the `content_tsv` trigger function to the weighted formula and recomputes **every** `khora_chunks` row (`UPDATE ... SET content_tsv = NULL`; the BEFORE trigger is the single definition of the formula). The transaction takes `ACCESS EXCLUSIVE` on `khora_chunks` at the `DROP TRIGGER` step and holds it to COMMIT, so **reads are blocked for the full duration of the rewrite** — recall stalls on every namespace, and concurrent services booting with `run_migrations=True` fail after 60s on the advisory lock. Run it out-of-band, not inside a rolling deploy. Mixed-version window: any instance on an older khora reinstalls the content-only trigger at `connect()` (this includes sibling services pinned to older khora sharing the database — for them the window stays open until the pin moves). GIN index unchanged; expect table bloat on the order of the table's size until autovacuum catches up (same shape as 044). `downgrade()` is symmetric, complete, and equally expensive.
- **Embedded (`sqlite_lance`) upgrade posture: recreate, not migrate.** Fresh embedded databases get the title-indexed FTS shape from connect-time DDL. Pre-existing embedded databases keep content-only FTS silently — there is deliberately no SQLite migration, ALTER, or backfill; recreate the database and re-ingest to enable title search. A connect-time shape probe (`fts_has_title`) gates the weighted scoring and drives the degradation above.
- Dependency bumps: `crewai` 1.14.4 → 1.15.10 (clears mcp CVE-2026-52870 / CVE-2026-52869 / CVE-2026-59950 and json-repair), `cryptography` 48.0.1 → 50.0.0, `pyopenssl` 22.0.0 → 26.0.0.

## [0.25.0] - document enumeration surface, SQLite migration-cascade fix, SurrealDB timestamp-bind fix, documents schema hardening

### Added

- **Keyset document enumeration** (#1576–#1600, ADR-123 V1): `Khora.list_documents` becomes a full keyset walk over a namespace's documents, newest first (`created_at DESC, id DESC`). Beyond the pre-existing `namespace` / `limit`, it now accepts `filter` (a `RecallFilter` or its dict/wire form; the event-time key `occurred_at` is not enumerable and raises), `status` (a `DocumentStatus` or its string; an unknown value raises rather than matching nothing), `updated_before` (half-open `updated_at < bound`), and `after` (a keyset resume position — a `DocumentCursor` or any returned `Document`). Two new public types are exported from the top-level package (`khora.__all__`): **`DocumentPage`** — the return type, a `Sequence[Document]` carrying walk-control metadata (`next_after`, `exhausted`, `post_filtered_keys`) — and **`DocumentCursor`**, the opaque resume position. Drive the walk on `while not page.exhausted`, feeding `page.next_after` back as `after`; a short page is **not** an end signal. There is no `offset`.

### Breaking changes

- **`Khora.list_documents` return type changed from `list[Document]` to `DocumentPage`.** `DocumentPage` subclasses `collections.abc.Sequence`, so iteration, `len()`, integer indexing and truthiness remain compatible; slices return a tuple-backed sequence rather than a list. A caller that only iterates or integer-indexes the result is source-compatible. What breaks: code that relied on the concrete `list` type — mutation (`.append`/`.extend`/`+=`), `list`-identity checks (`type(r) is list`), concatenation with `+ [...]`, or a type annotation of `list[Document]`. Wrap in `list(page)` at the boundary if a real list is required. (ADR-123 V1 edge case #13.)
- **`MemoryEngineProtocol.list_documents` signature and return type changed — breaking for out-of-tree engine implementations.** In `engines/protocol.py` the method went from `list_documents(namespace_id, *, limit=100) -> list[Document]` to `list_documents(namespace_id, *, filter_ast=None, status=None, updated_before=None, limit=100, after=None, scan_bound=None) -> DocumentPage`. khora is OSS (ADR-090) and third parties implement `MemoryEngineProtocol` directly, so an engine defined outside this repo must update its `list_documents` to the new keyword-only parameters and `DocumentPage` return type or it no longer satisfies the protocol. The engine threads the pre-resolved arguments through to the storage coordinator unchanged and must not interpret `filter_ast` (the facade validated and lowered it; the coordinator compiles and applies it). Same engine-boundary discipline as ADR-092. All in-tree engines are already updated.
- **`remember` / `remember_batch` / `submit_batch` with `source_type=""` now persist `"library"`.** Previously the empty string was stored verbatim. This is a silent behaviour change on the primary public entry point — a caller that deliberately stored `""` as a sentinel will read back `"library"`. It is consistent with migration 037, which already rewrote existing `''` rows to `'library'` and moved the column default off `''`. `""` still round-trips unchanged on the two un-normalized surfaces below.
- **`update_document` and `partial_update_document` now raise `IntegrityError` on `source_type=None`** where they previously wrote a silent NULL. Both compile to a Core `update().values(...)`, where no column default applies and the `None` reaches the database as SQL NULL. `partial_update_document` additionally carries `source_type` in its public field whitelist and bypasses `Document` entirely. `kb.storage` is stable public API, so this is recorded as breaking; the raise is the intended outcome for what was always a type violation (`Document.source_type` is annotated `str`), and no code was added to paper over it.
- **`create_document` is NOT affected, despite taking the same `Document`.** `DocumentModel.source_type` carries a Python-side `default="library"`, and SQLAlchemy applies a column default on INSERT when the value is `None` without distinguishing "unset" from "explicitly `None`". So `Document(source_type=None)` → `create_document` silently stores `'library'` rather than raising. `Document(source_type="")` stores `''`, which the constraint permits. The asymmetry between create and update is unintuitive and is called out here deliberately.
- **Deploy ordering for migration 055:** ship the call-site normalization before running the migration. A live instance on the old code writing a NULL `source_type` while the revision is in flight aborts it and forces a retry. Old code against the new schema is fine; it only reads the column.

### Fixed

- **SurrealDB timestamp filters compared against nothing, and two features were inert because of it.** Four sites in `storage/backends/surrealdb/relational.py` bound a `datetime` operand as an ISO-8601 string via `.isoformat()` while the columns are `TYPE datetime`. SurrealDB compares across types by type rank, **without reaching the values**: `datetime < string` is always false and `datetime >= string` always true, for any string — verified with a past date, a future date, one exactly equal, the non-timestamp `'zzz'` and the empty string. A string bind therefore does not misorder a predicate, it *pins* it, silently and without error, and the direction of the failure depends on the operator. Each site now binds the `datetime` object, matching the adapter's own write path. Measured on python SDK 2.0.0, identical on `memory://` and `surrealkv://` (both on the SDK-bundled core 2.3.10, read off the extension binary rather than inferred from the Python package version) and a standalone `ws://` server 2.3.7 — every leg is 2.3.x, so the behaviour is measured on **2.3.x only**, not below 2.3 and not on 3.x. Three consequences, the first a plain bug and the other two behaviour that now activates on upgrade. (1) `list_documents(updated_before=…)` returned an empty page on every call — measured 0 rows where the object bind returns 3 of 6 — so a caller draining a namespace under a TTL bound concluded the namespace was empty. (2) The stale-PENDING dedup exclusion used `>=` and was pinned true, collapsing the clause to the legacy `status != 'failed'` path and making it byte-equivalent to the feature being off: **a PENDING row left by a crashed ingest was a permanent dedup hit, so that document could never be re-ingested** — no error, no log, the content simply never came back. Such documents now re-ingest on their next submission; fresh PENDING rows still dedup, so the concurrent in-flight guard is unaffected. (3) `claim_orphaned_documents` returned zero rows on every call, so no orphaned document was ever reclaimed on this backend. Its SELECT and its UPDATE are one change and cannot be split: the UPDATE wrote a string into the same `TYPE datetime` column, and SurrealDB **rejects** that (`InternalError: … expected a datetime`) rather than coercing it, discarding the whole statement so a multi-field UPDATE leaves every field untouched — it was unreachable only because the SELECT ahead of it returned nothing, and fixing the SELECT alone would have converted a silent no-op into a hard error on the first claimed orphan. Because the write was rejected and never coerced, **no existing row can hold a string timestamp and no repair is needed or possible.** **Operator impact on `backend: surrealdb`:** the first orphan sweep after upgrading claims the entire accumulated backlog, one LLM extraction per document, so size it beforehand by counting PENDING/PROCESSING rows older than the configured cutoffs (defaults 5 minutes and 900 seconds) — this applies only if the pending processor is actually started. Both activated features derive their cutoff from the same grace period, so a single stale PENDING row is simultaneously a dedup miss (a resubmit mints a new row, the checksum index not being UNIQUE) and claimable by the sweep (which processes the old row) — two extractions for one piece of content, pre-existing on PostgreSQL but switching on all at once here. The claim is also not atomic on this backend (SELECT then per-row UPDATE, no locking, where PostgreSQL uses `FOR UPDATE SKIP LOCKED`), so concurrent processors can double-claim; that was unreachable before and is now live. Unchanged by this fix but worth knowing: both the claim query and the dedup query carry a top-level `OR` that costs them an index — `EXPLAIN` reports `Iterate Table` identically before and after — and the dedup query is on the `remember()` hot path. Not affected: PostgreSQL, `sqlite_lance` and the `backend: sqlite` stack, whose `.isoformat()` is correct because it stores `TEXT`; and the filter compiler's metadata binds, which are a genuinely separate case (metadata round-trips through a `FLEXIBLE object` column that preserves the written type, gated with `type::is::string`). A sweep of the rest of the package found no further site. The `namespace_id` predicate on `claim_orphaned_documents` and on both checksum reads had no test pinning it — it could be neutralized with the whole suite green — and now does; on the claim path the guard asserts foreign rows are neither returned nor re-stamped, that path being a write.
- **Embedded-SQLite migrations no longer delete child rows.** `db/migrations/env.py` issued `PRAGMA foreign_keys = ON` on the SQLite migration connection for the whole run. Alembic's batch mode implements "alter column" as SQLite's documented table-rebuild procedure — create a temp table, copy rows, `DROP TABLE`, rename — and with FK enforcement on, that `DROP TABLE` performs an implicit `DELETE FROM` that fires every inbound `ON DELETE CASCADE` before the rename puts the table back. Measured on a populated embedded database upgraded to head: `(documents, chunks, keyword_chunks, chronicle_events, chunks_fts)` went from `(1, 1, 1, 1, 1)` to `(1, 0, 0, 0, 0)` — the cascade is transitive. Affected rebuilds across the chain, with their cascade radius measured against the live schema at head (24 tables): `documents` (revisions 016 / 037 / 055 / 056) reaches `chunks` directly and `keyword_chunks` / `chronicle_events` through it, `entities` (008) reaches 3, and `memory_namespaces` (001 / 010 / 011 / 012 / 013) reaches 13 directly and 14 transitively — every table holding documents, chunks, entities or events. The only tables it does not reach are `permissions`, `khora_dream_runs`, `khora_hook_subscriptions`, the Alembic version table, and the FTS shadow tables (which empty anyway through the `chunks` triggers). Worse, starting from revision `000`, migration 010's `DROP TABLE memory_namespaces` fails outright with enforcement on and child rows present — `IntegrityError` or `SQLITE_CORRUPT` ("database disk image is malformed"), seed-dependent — the per-migration transaction rolls back, and the database is left **stranded at its starting revision**, unable to advance (the file itself is intact; `PRAGMA integrity_check` is clean). **A database stranded that way is fully recovered by this change**: re-running the upgrade on this version completes the chain with every row intact, because the revision that failed rolled back before writing anything. This is the one case the fix repairs rather than merely prevents — if your embedded upgrade is currently failing, upgrade to this version and re-run it rather than rebuilding the database. The pragma is now explicitly `OFF` for the migration connection; it cannot be scoped narrower, because the pragma must be issued outside a transaction to be reliable and migrations run under `transaction_per_migration=True`. **This change prevents future loss. It does not recover anything a prior upgrade already destroyed.** The full set of revisions that can have caused loss on an embedded database is every batch-altering revision that shipped before this one — 001, 008, 010, 011, 012, 013, 016, 037 and 055 — not just the `documents` rebuilds. What an operator may be missing depends on which of them their database passed through while populated, and each one destroys only what existed at that point in the chain — the counts below were measured by building the chain to each revision, not by reading the constraint graph at head. The `documents` rebuilds: 016 destroys `chunks`; 037 destroys `chunks` and `chronicle_events`; 055 destroys `chunks`, `chronicle_events` and `keyword_chunks`. The `entities` rebuild (008) destroys `relationships`, `temporal_edges` and `time_edge_links`. The `memory_namespaces` rebuilds are the severe ones: 001 destroys 8 tables and 010 / 011 / 012 / 013 destroy 11 each — every table holding tenant data at that point in the chain. (A `memory_namespaces` rebuild against today's schema would reach 14, but no revision does that; the smaller historical numbers are the ones that describe real damage.) Deleted rows are gone — there is nothing in this change that brings them back — and on the `sqlite_lance` stack LanceDB still holds the embeddings keyed by the deleted chunk ids, so the residue is orphaned vectors rather than merely missing rows. **How to tell whether you are affected.** Do *not* rely on `PRAGMA foreign_key_check` — it reports **clean** on a damaged database, because a cascade deletes children precisely so that no violation remains, so the obvious check falsely reassures. Check row counts instead: `SELECT COUNT(*) FROM documents` greater than zero alongside `SELECT COUNT(*) FROM chunks` equal to zero is the near-certain signature of a `documents` rebuild having cascaded. Comparing relational chunk counts against the vector store detects the same loss on the `sqlite_lance` stack, but note it only sees the `chunks` loss — a database that crossed 001 or 010–013 while populated lost up to eight further tables (entities, relationships, episodes, memory events and facts, temporal edges and time nodes) that no vector-store comparison will surface, so count those directly too. A database that was empty when it passed through those revisions is unaffected, which is the common case for anyone who migrated before ingesting. What is given up: the migration run no longer rejects FK violations it would previously have caught, so a future revision that deleted parent rows would orphan children silently instead of cascading. Nothing in the chain does that today; the constraints themselves are unaffected and are still enforced on application connections. Nothing caught this earlier because every existing migration/drift gate builds the chain on an **empty** database, where a zero-row cascade is invisible — a populated-database regression test now closes that gap.
- **`documents.created_at` is NOT NULL** (migration `056_documents_created_at_not_null`): the column has been physically nullable on both dialects since migration 000 (`nullable=False` was never emitted, and nothing in the chain altered it since) while `DocumentModel.created_at` has declared it non-optional all along. A NULL breaks any total order over `(created_at DESC, id DESC)` in two ways: SQLite sorts NULLs last under `DESC` and PostgreSQL sorts them first, so one namespace paginates differently per backend; and a keyset cursor predicate over the pair evaluates to NULL for such a row, so it is silently skipped on every page — never returned, never an error. Existing NULL rows are backfilled from `updated_at` where that is present (it is nullable too, so the fallback branch is reachable) and stamped with the Unix epoch otherwise; both counts are logged separately on `khora.migration.applied` so the invented subset is auditable. **The backfill is not reversible** — `downgrade()` drops the constraint only. An epoch-stamped row sorts dead last under `created_at DESC`, which is visibly wrong but reachable; today the same row is invisible. NOT NULL rules out NULL; it does not make an epoch-stamped timestamp correct. No call-site change has to ship first: unlike 055, no reachable write path emits a NULL `created_at` (`created_at` is excluded from the partial-update whitelist, is absent from the full-update column set, and the ORM's Python-side default fires on an explicit `None` at INSERT). Not affected: the `backend: sqlite` stack, whose hand-written `documents` DDL has always carried `created_at TEXT NOT NULL` and whose single INSERT path coalesces a missing timestamp to now; and SurrealDB, whose `created_at` field is `TYPE datetime` (not `option<datetime>`) and so already rejects `NONE`. Both are recorded in place rather than changed. **"Not affected" means the NOT NULL invariant already held there — it does not mean their ordering matches.** The `backend: sqlite` tier writes `datetime.isoformat()` into a `TEXT` column and compares it lexicographically, so it stores an offset-carrying, `T`-separated string whose sort order diverges from the instant order across rows written at differing UTC offsets, and diverges from Postgres regardless. That is a pre-existing property of a separate backend, stated rather than closed: this release neither introduces nor repairs it, and the ordering guarantee above is scoped to the two Alembic-managed tiers.
- **`documents` ORM/live-schema drift** (migration `055_documents_source_type_alignment`): two declarations in `DocumentModel` that the Alembic chain never actually built are now built. (1) `ix_documents_namespace_source_type (namespace_id, source_type)` was declared in `__table_args__` but created by no migration — it is created now with `IF NOT EXISTS`, because `optimize_storage()` already ships the identical `CREATE INDEX IF NOT EXISTS` and a bare create would wedge the chain on any database that ran it. This is a declaration/schema alignment, **not** a performance change: no production query path filters `documents` by `(namespace_id, source_type)` today. (2) `documents.source_type` was `nullable=False` in the ORM but NULL-able in the live schema on both dialects — migration 000 omitted `nullable=False` and migration 037's `existing_nullable=False` was a renderer hint, not a constraint. Existing NULL rows are backfilled to `'library'` (the ORM default) before the flip; that backfill is **not reversible**, and `downgrade()` drops an index `optimize_storage()` may have created independently. `NOT NULL` does not exclude the degenerate `''` value, which is what the codebase historically produced.
- **Explicit `source_type=None` no longer reaches the `documents` write.** `dict.get("source_type", default)` returns `None` when the key is present with a `None` value — the default only fires on key *absence* — so a caller passing `source_type=None`, or a per-doc dict carrying `None`, previously wrote a NULL that the ORM's own declaration said was impossible. Every write path reached through `remember` / `remember_batch` / `submit_batch` or the ingest pipeline (including the VectorCypher / Chronicle / Skeleton batch paths) now collapses `None` and `''` to the applicable default, at both the top-level kwarg and the per-doc dict value. `khora_chunks.source_type` is untouched — that denormalized copy is deliberately nullable, as is SurrealDB's `option<string>` field.

### Migration

- **Migration 056 blocks `documents` for the duration of its heap scan.** `ALTER TABLE ... SET NOT NULL` takes an `ACCESS EXCLUSIVE` lock (blocks reads *and* writes) and performs a full heap scan to verify no NULL remains. Unlike 055 there is no index build, so the scan is the whole blocking window; order of magnitude, tens of seconds at ~10M rows. The revision's `SET LOCAL lock_timeout = '5s'` bounds how long the statement waits to *acquire* the lock — never how long one is *held*, and never the scan. **To skip the scan on a large table (PostgreSQL 12+),** add a validated CHECK before deploying — `ALTER TABLE documents ADD CONSTRAINT tmp_documents_created_at_nn CHECK (created_at IS NOT NULL) NOT VALID;` then `ALTER TABLE documents VALIDATE CONSTRAINT tmp_documents_created_at_nn;` (the validation takes only `SHARE UPDATE EXCLUSIVE`) — and drop it afterwards. This is deliberately not done inside the migration: under `transaction_per_migration=True` the `ADD CONSTRAINT`'s own `ACCESS EXCLUSIVE` is held to COMMIT anyway, so in-transaction it buys nothing, and getting the benefit would require an autocommit block and the partial-apply state that comes with it. The supported floor is PostgreSQL 11, where the skip does not exist and the scan always runs. On SQLite the flip goes through `batch_alter_table`, whose table rebuild is only non-destructive because FK enforcement is now off for the migration connection (see above).
- **Migration 055 blocks `documents` for the duration of the revision.** `ALTER TABLE ... SET NOT NULL` takes an `ACCESS EXCLUSIVE` lock (blocks reads *and* writes) and performs a full heap scan to verify the invariant; the non-concurrent `CREATE INDEX` takes a `SHARE` lock (blocks writes). Because the revision is a single transaction, both locks are held until COMMIT, so the windows add. The migration's `SET lock_timeout = '5s'` bounds how long each statement waits to *acquire* a lock — it does not bound how long one is *held*, nor the heap scan or index build. Order of magnitude: tens of seconds at ~10M rows, minutes at ~100M. **To shrink the window on a large table,** run `CREATE INDEX CONCURRENTLY IF NOT EXISTS ix_documents_namespace_source_type ON documents (namespace_id, source_type);` before deploying — the migration passes `if_not_exists=True`, so its own create then becomes a no-op and only the `SET NOT NULL` scan blocks.
...
- **Bi-temporal soft-delete columns are now ACTIVE on read** (#1291, #1272, closes #970): `valid_to` / `invalidated_at` / `valid_until` on `entities` / `relationships` (reserved write-only since #888) are now filtered by the recall read paths. pgvector `list_entities` / `list_relationships` apply `_entity_live_filter()` / `_relationship_live_filter()` and Neo4j `list_entities` / `list_relationships` filter `valid_until` unconditionally, so dream-pruned edges, soft-retired entities, and dedupe self-loop invalidations now disappear from recall. This changes the default recall shape; it is correct only because the new dream graph-mirror keeps PG and the graph consistent. Callers that relied on retired rows still surfacing must read the historical paths explicitly.
- **`RecallResult.usage` renamed to `RecallResult.llm_usage`** (#1355): aligns with `RememberResult.llm_usage`. Callers reading `result.usage` on a recall must switch to `result.llm_usage`.
- **`GraphBackend.create_relationships_batch` return type changed from `int` to `list[tuple[Relationship, bool]]`** (#1327): mirrors `upsert_entities_batch`'s `(entity, is_new)` contract across the protocol and every backend so dispatch sites can emit `relationship.created` on a genuine create and `relationship.updated` on a dedup-merge, carrying the canonical persisted edge id. On a merge the input relationship's `id` is synced in place to the stored edge id. The int-count callers (replace path, expansion, batch-inference) take `len(results)`. Neo4j/Memgraph report `is_new` exactly; pgvector/sqlite_lance report it from the relationship-`id` collision they key on; SurrealDB/Neptune/AGE report best-effort `True`.
- **Binary readers raise `ExtractionError` on parse failure instead of returning `""`/`None`** (#1344, #1233, #1229): `extract_xlsx_text` / `extract_docx_text` / `extract_parquet_text` / `extract_if_needed` (the documented stable public boundary) now raise `khora.exceptions.ExtractionError` on genuine parse/format failures rather than silently returning empty, so a corrupt file is distinguishable from a legitimately-empty one. Legitimately-empty content still returns `""` / `None`. Also fixes an fd leak on malformed xlsx by guaranteeing `wb.close()` via `try/finally`.

### Added

- **Neo4j dream tombstone-mirror + PG/Neo4j read filter + reconciler** (#1291, #1272): a post-commit step folds the three PG soft-delete columns onto the graph's single `valid_until` via capability-gated bi-temporal verbs, runs outside the apply transaction (eventual consistency, idempotent by id), and on failure increments `khora.dream.graph_mirror.partial_failure`, appends a `Degradation` to `DreamResult.metadata`, and queues a `graph_mirror_pending` row the reconciler drains at the next apply start.
- **GraphBackend bi-temporal dream verbs + capability probe** (#1289, #1271): `supports_dream_mirror() -> frozenset[OpKind]` plus `soft_invalidate_relationships_batch`, `soft_retire_entities_batch`, `rewrite_relationship_endpoints_batch`, `rename_types_batch` on the protocol. `GraphBackendBase` defaults raise `DreamBackendUnsupported` (honest capability flags + structured skip, never a destructive hard-delete fallback); Neo4j ships the native, dream-predicate-keyed implementation.
- **Crash-durable graph-mirror reconciler** (#1295, #1292): the `graph_mirror_pending` entry is now written in the same transaction as the checkpoint advance (closing the hard-crash window between PG commit and mirror), the drain is namespace-scoped (a later run with a fresh `run_id` heals a prior run's failures), and the unsupported-op skip, namespace-resolve error, and pending-read failure all surface structured ADR-001 records instead of diverging silently.
- **Dream graph-mirror reversed on undo** (#1297, #1275): new `restore_entities_batch` / `restore_relationships_batch` / `restore_relationship_endpoints_batch` reverse verbs plus `extract_unmirror_targets` / `unmirror_targets` make `dream_undo` converge both stores back to byte-identical pre-apply live sets (clearing `valid_until`, deleting the `:EntityVersion` snapshot, re-pointing endpoints), with a `khora.dream.graph_unmirror.partial_failure` counter; previously undo half-reverted (PG only) and re-diverged the graph.
- **Dedupe entity-merge edge re-pointing mirrored to the graph** (#1293, #1273): the dedupe `previous_relationships` snapshot is translated into the `rewrite_relationship_endpoints_batch` verb with a global union-find `absorbed -> canonical` remap, self-loop exclusion, reconciler parity, and idempotent convergence, so graph recall no longer keeps edges pointing at a retired endpoint after a merge.
- **Dream communities materialized to the graph and projected into recall** (#1294, #1309, #1276, #1308): `materialize_communities_batch` MERGEs `:Community` nodes + `[:HAS_MEMBER]` edges (idempotent on `community_id`) from the previously-readerless `khora_dream_communities` table, with `get_communities` / `get_entity_communities` readers on the protocol, coordinator, and `Khora` facade; VectorCypher then projects the matched entities' communities onto a new `RecallResult.communities: list[CommunityNode]` field (deduped, capped at 10, shallowest-first). `CommunityNode` is exported from `khora.core.models` and reachable via `RecallResult.communities`.
- **Per-backend dream-apply on the embedded and unified stacks** (#1299, #1300, #1301, #1298, #1310): the #875 dialect gate is lifted so dedupe / prune_edges / normalize_schema apply on `sqlite_lance` (with UUID-bind safety and the embedded live-set read filter) (#1299); SurrealDB-unified gains a native SurrealQL apply path + flat `valid_until` soft-delete mirror (#1300); Memgraph gains a flat soft-delete mirror and is no longer overwritten by a Neo4j backend (#1301); Neptune and AGE gain a capability-gated soft-delete-only (prune_edges) mirror (#1298); and AGE additionally folds its soft-delete into the dream-apply PG transaction atomically (#1310). SurrealDB dedupe also re-points incident edges onto the canonical (#1304, #1303).
- **DreamRunStore abstraction + non-PG run-state** (#1290, #1274): run-state (checkpoint/resume, status, history) is factored into a `DreamRunStore` protocol with Postgres / SQLite-sidecar / SurrealDB implementations, so resume and status/history work on non-PostgreSQL stacks; migration 047 adds the `graph_mirror_pending` JSON column the reconciler builds on.
- **Cross-store live-set permutation invariant gate** (#1296, #1268): a CI gate asserting that after a dream apply on a real pg+Neo4j stack the graph-preferring read path and the PG ground-truth live set return byte-identical id sets across every mirrorable op kind, and that a second convergence pass emits zero work.
- **Opt-in dream LLM-gated contradiction/dedupe reconciliation** (#1306, #1281): promotes the report-only contradiction op to a two-LLM-judged (`gpt-4o-mini` + `claude-haiku-4.5`) reconcile op that soft-deletes the losing edge only on grounded, both-judges-agree invalidation (else defers, no mutation), mirrors the soft-delete to the graph, writes a `dream_conflicts` triage row (migration 048), respects the dream token budget, and is default-OFF behind `DreamConfig.contradiction_reconcile_enabled`.
- **LLM cost tracking + `UsageSummary`** (#1355): a best-effort `cost_usd` field on `LLMUsage` (litellm pricing tables, `0.0` on unknown models) threaded through all 10 LLM call sites, plus a public `UsageSummary` aggregation type with per-operation / per-model breakdowns. `record_usage` is added to chronicle compression/events and cross-encoder reranking.
- **Opt-in Tier-2 LLM temporal-category fallback** (#1318, #981): `TemporalDetector.detect_async()` consults a one-shot LLM classifier only when the keyword tier returns `NONE`, resolving multilingual and paraphrased temporal intent. Default-OFF behind `KHORA_QUERY_TEMPORAL_SEMANTIC_FALLBACK_ENABLED` (zero LLM cost when off); degrades to the keyword result on any failure with a structured `Degradation` + `khora.vectorcypher.temporal_semantic_fallback.degraded_total`.
- **Persistent hook subscriptions durable across restarts** (#1326, #599): an opt-in PostgreSQL/SQLite-backed durable layer (table + migration 049, `HookSubscriptionStore`) reloaded on `Khora.connect()`, with `Khora.subscribe_persistent` / `unsubscribe_persistent`. Default `None` store keeps the in-process fast path byte-identical and zero-cost; a persist/load failure degrades (keeps the sub in memory + ADR-001 `Degradation`) rather than dropping it.
- **BM25 full-text search on the Weaviate and turbopuffer backends** (#1342, #1338): `search_fulltext` now natively implements BM25 (`collection.query.bm25` / `rank_by=("content","BM25",…)`) on both stores, closing the silent-empty `recall(mode=KEYWORD)` path; both mirror their `search()` date push-down + recall-filter handling.
- **HyDE and MMR diversity wired into the default `recall()` path** (#1321, #1018): `enable_hyde` / `hyde_num_hypotheticals`, `enable_diversity` / `diversity_lambda`, and `stage1_recall_limit` from `QuerySettings` now take effect on the VectorCypher `recall()` path (previously inert because `recall()` bypasses `QueryEngine`); `enable_diversity` defaults `True`, so MMR + stage-1 over-fetch run by default. `enable_entity_linking` stays documented-inert on this path.
- **Configurable abstention knobs + calibrated confidence** (#1333, #1331): `query.abstention_*` thresholds/weights and an `abstention_mode` (default `cosine_floor`, which lets the topicality floor decide on its own; `weighted` restores the old formula) are exposed on `KhoraConfig`, plus a calibrated `engine_info["confidence"]` in `[0,1]`. Fixes the prior weighting where the only relevance signal on a populated namespace could never fire.
- **Opt-in independent BM25 lexical channel** (#1332, #1330): `enable_bm25_channel` is exposed via `KHORA_QUERY_ENABLE_BM25_CHANNEL` (default False); the SQLite FTS5 path now OR-joins tokens (aligning with pgvector) so a natural-language query embedding an exact ID matches, with an ADR-001 empty-multitoken-channel degradation.
- **Absolute cosine relevance on VectorCypher `RecallChunk.score`** (#1319, #811): the final fusion step now reports each chunk's raw vector cosine similarity (an absolute, cross-query-comparable measure) instead of a per-result-set min-max rank where the top was always `1.0`, so callers can threshold meaningfully; ordering is unchanged and abstention still keys off the internal pre-fusion score.
- **VectorCypher emits entity/relationship semantic hooks on remember** (#1317, #978): `entity.created` / `entity.updated` (split on `is_new`) and `relationship.created` now fire on the VectorCypher write path, matching Chronicle; previously the semantic-hooks feature was a silent no-op on the default graph engine.
- **Reachable dream ops** (#1288, #1287, #1284, #1286): the vectorcypher dedupe / centroid-recompute / source-chunk-GC ops (#1265-#1267, #1263), the chronicle fact-compaction / event-clustering ops (#1067), and the vectorcypher schema-drift op (#1036, now threading `expertise=` through `Khora.dream`) are wired into `dream_capabilities` and `plan_dream` with default-OFF master switches and structured skip reasons (previously dead handlers that could never be planned). A per-run + per-namespace-per-day dream LLM token budget is now enforced (#1286, #1270).
- **`StorageCoordinator.temporal_store()` factory accessor** (#1261, #1262): a single seam that builds a freshly-connected temporal store and owns per-backend shared-resource wiring (pgvector engine, SurrealDB connection, sqlite_lance handle); the VectorCypher and Skeleton engines now call it instead of constructing stores and reaching into backend internals, cutting the engines' back-edge into the skeleton package.
- **Recall-filter coverage hardening** (#1246, #1245, #1235): a channel × filter-operator coverage matrix forcing each VectorCypher post-filter channel to fire and prove enforcement, backed by a hermetic registry meta-test (#1246); a Hypothesis property-based differential fuzzer comparing `compile_python` against the `sqlite_lance` read path plus metamorphic-rewrite invariants (#1245); and the boolean-composition corpus run through real ingest on the multi-channel e2e lanes (#1235).

### Changed

- **Declared `numpy` as a direct core dependency** (`numpy>=2.1`) (#1250): numpy is imported unconditionally on the eager `import khora` path but was previously pulled in only transitively via `sentence-transformers -> torch`.
- **`reranking` extra reduced to a no-op alias** (#1251): the extra re-declared `sentence-transformers` (already a core dependency), a manifest bug; it is now `reranking = []`, byte-equivalent to a bare install, with the cross-encoder reranker default unchanged.
- **Tier-1 disambiguation of whole-word ambiguous temporal keywords** (#1325, #981): `may` (month vs. modal/proper-noun) now fires EXPLICIT only in temporal context (temporal preposition or adjacent number); `just` narrowed to recency phrases; `active` narrowed to state-query noun phrases, killing the last whole-word false-positive bucket, in exact Rust/Python `_accel.py` lockstep with no khora-accel version bump.
- **Checksum dedup ordering made deterministic** (#1354): `stage_documents_batch` built its unique-checksum list from a `set` (`PYTHONHASHSEED`-dependent order); now uses `list(dict.fromkeys(...))`, behavior-identical but deterministic.
- **Project description aligned to the README** across `pyproject.toml`, `CONTRIBUTING.md`, and the architecture overview (#1252).
- **Security dependency bumps.** pyo3 and the `rust-numpy` crate `0.28 -> 0.29` (#1254), resolving the high-severity out-of-bounds-read advisory GHSA-36hh-v3qg-5jq4 in `PyList`/`PyTuple` iterators (the shipping cdylib builds clean with no source changes); and cryptography `48.0.0 -> 48.0.1` (GHSA-537c-gmf6-5ccf, vulnerable bundled OpenSSL), python-multipart `0.0.27 -> 0.0.32` (GHSA-5rvq-cxj2-64vf, quadratic querystring-parsing DoS), and pyjwt `2.12.1 -> 2.13.0` (GHSA-xgmm-8j9v-c9wx, JWK-as-HMAC-secret algorithm confusion, plus four further bundled JWT fixes) pinned via the existing `constraint-dependencies` block (#1253, #1244).
- **Temporal vector-store layer relocated out of the engines package** (#1255, #1256, #1257, #1259): the neutral temporal data types move to a strict-leaf `khora.core.temporal` (`TemporalChunk`, `TemporalSearchResult`, `ChunkTemporalFilter`) and the `TemporalVectorStore` protocol + `create_temporal_store` factory + the five backend implementations move to `khora.storage.temporal`, fixing the layering direction (these packages import nothing from `khora.engines.*`); core-chunk PageRank selection moves to a leaf `khora.core.ranking`. The legacy `khora.engines.skeleton.backends.*` paths remain working as identity-preserving re-export shims that emit `DeprecationWarning`.

### Fixed

- **Word-boundary-aware temporal classifier** (#1313, #1285, #981): the classifier matched keywords as raw substrings, so `changed` fired inside `unchanged`/`exchanged` and `march` inside `marched`; matching is now `\b`-anchored in both the Rust kernel and the Python fallback (lockstep, no version bump), and the stale possessive STATE_QUERY assertion is corrected.
- **Future-dated recency clamp + counterfactual veto gate** (#1312, #1230, #1227): a future-dated chunk no longer produces a recency factor above `1.0` that boosts it past present-dated chunks (`days_old` clamped at 0), and the counterfactual-query veto now fires on the anti-recency token directly instead of being gated on the wrong (`temporal_recency_floor_enabled`) flag, so a channel-on/floor-off combo no longer injects today's chunks into counterfactual queries.
- **Cross-tenant read isolation closed across the graph backends (IDOR family)** (#1335, #1336, #1339, #1240, #1248): the legacy `sqlite` backend's four read-by-id methods now require and filter `namespace_id` (#1335); `StorageCoordinator` public read methods resolve the documented *stable* namespace id to the row id before forwarding (a stable-id `list_relationships` previously returned a silent empty set) (#1336); Neo4j/Memgraph relationship-MERGE scopes both endpoint MATCHes to `namespace_id` and dedupes intra-batch duplicates (#1339); and `list_relationships` constrains both endpoints to `(:Entity {namespace_id})` and skips-and-warns on null endpoint ids instead of crashing, across Neo4j (#1240) and Memgraph/Neptune (#1248).
- **Chronicle recall and write fixes** (#1316, #1314, #1315, #1311, #1287): chronicle event/fact writes on `backend=surrealdb` are now atomic (all-or-nothing per batch, with ADR-001 degradation on failure) instead of a partial per-row write reported as 0 (#1316); `forget()` / re-ingest on SurrealDB actually deletes old chunks (the write path now populates the scalar `namespace_id` the delete filters on) (#1314); the entity-channel relevance gate no longer lets an embedding-less chunk out-rank a cosine-verified one, and an incidental `20xx` token no longer triggers a spurious date filter (#1315); the entity dedup-merge path preserves the real-world `valid_from` instead of lowering it to ingest time (#1311); and the chronicle fact-compaction/event-clustering UUID binds use the SQLite hex form so they match rows (#1287).
- **Recency-channel provenance filter leak closed** (#1236, #1223): the VectorCypher recency channel rebuilt chunks into provenance-less `Chunk` objects and post-filtered against the blanks (so a `$ne` leaked and a `$eq` silently dropped rows, while reporting the filter as enforced); the filter is now pushed into the `khora_chunks` SQL `WHERE` and reported honestly.
- **HyDE silent-fallback observability** (#1343, #1324, #1323, #1322): `HyDEExpander.expand_query_embedding` now records a structured `Degradation` (`khora.query.hyde.degraded_total`) and logs with `exc_info=True` on a degrade-to-original-embedding, instead of falling back silently.
- **LLM cost-budget accounting fixes** (#1349, #1348, #1224, #1231): the Level-2 evaluator no longer charges the per-subscription token budget on a namespace-cap rejection (both caps are checked read-only before either is charged) (#1349), and `LiteLLMEmbedder.embed()` no longer double-counts a single cache miss (#1348).
- **Thinking-model extraction truncation + provider api-key derivation** (#1337): entity extraction now detects `MAX_TOKENS` (not just OpenAI's `length`) case-insensitively, floors the token budget for known thinking models (Gemini 2.5, o1/o3), auto-retries once with a doubled budget, and surfaces persistent truncation as an ADR-001 `Degradation` rather than a silent zero-entity success; `api_key_env` is auto-derived from the model prefix when left at the OpenAI default so a non-OpenAI model no longer silently reads the wrong key.
- **Exact-array `$eq` recall crash on a scalar metadata node in sqlite_lance** (#1234): the exact-array equality emitter ran the inner `json()` parse unconditionally, so a metadata path holding a scalar string raised `malformed JSON` and aborted the recall; it is now `CASE`-gated to run only when the node is a JSON array.
- **khora-accel `cargo test` compilation** (#1283, #1269): the `#[cfg(test)]` blocks in `community.rs` / `pagerank.rs` called removed pyo3 0.29 APIs; the GIL-free logic is extracted into `*_inner` fns the tests call directly, restoring the Rust-side determinism regression gate.
- **Filter-conformance seed grouping by `external_id`** (#1219): the seeder writes one `Document` per distinct `external_id` (the production chunking shape) instead of a 1:1 record→Document mapping that collided with the `documents` UNIQUE constraint, so the previously-pruned operator-coverage cases now run on every Document-path backend.
- **`Document.created_at` kept at ingest time on the chronicle batch path** (#1215, #993): `stage_documents_batch` piped the caller's real-world `source_timestamp` into the khora-ops `created_at` / `updated_at` columns (a cross-axis violation that corrupted backfilled-data timestamps and diverged from the single-`remember()` path); the assignment is removed so both columns fall to the ingest-time default.
- **sqlite_lance temporal correctness** (#1346, #1068, #1070, #1182, #1305, #1302): `occurred_at` / `source_timestamp` are UTC-normalized before storing as TEXT so a non-UTC offset no longer lexically sorts an in-window chunk out of range; the Skeleton engine's `stats().chunks` now counts the temporal-store `khora_chunks` table; `search_recent_chunks` is implemented (the recency channel was a base-class no-op) (#1346); and the embedded recursive-CTE traversal (`find_paths` / `get_neighborhood`) now applies the same soft-delete live-set filter as `list_relationships` under `prefer_current` (#1305).
...
- **`engine_info["filter"]` is now the engine's `FilterPushdownReport` verbatim** (#1069, #1112): the recall facade previously overwrote it with a flat `{engine, supported, pushed_down}` summary; it now passes the engine's report through unchanged. The `supported` / facade-level `engine` keys (never part of the canonical schema, read nowhere) are gone, and engines that do not report filter pushdown now **omit** the `"filter"` key entirely rather than carrying a synthesized default — callers that read `engine_info["filter"]` must treat it as optional (key on its presence).
- **Chronicle `pushed_down` now uses ALL-leaves semantics** (was "any key pushed") (#1179): `RecallResult.engine_info["filter"]` on the Chronicle engine now carries the full canonical report (`pushed_keys` / `post_filtered_keys` plus the per-channel `channels` breakdown, single `chunks` channel) instead of the hand-rolled `{pushed_down, post_filtered}` two-field dict. Chronicle pushes only the `source_timestamp` date bound into the recency window while always enforcing the full predicate in memory, so a multi-predicate filter now reports `pushed_down=false` with the pushed/post-filtered keys split out — exposing the partial-pushdown truth the old boolean hid. Callers that read `pushed_down` as a proxy for "was any filter work pushed down" should read a non-empty `pushed_keys` instead.
- **Recall filters over unbacked SurrealDB and turbopuffer system keys now fail loud** (#1111, #1097): the SurrealDB skeleton store backs only `occurred_at` / `created_at` with real columns; a filter on any of the other eight denormalized document keys previously compiled to a total-false predicate that silently dropped all results. It now raises `RecallFilterUnsupportedError` for any operator on an unbacked key (`$exists` included), gated on the declared `field_mapping` whitelist (#1111). The turbopuffer backend, which ignored `filter_ast` on every search path, now raises `RecallFilterUnsupportedError` for any constraint-bearing filter (a constraint-free `filter={}` still passes through unchanged) (#1097).

### Added

- **`FilterPushdownReport` / `FilterChannelReport` public API** (#1069, #1112): the structured shape of `RecallResult.engine_info["filter"]` is now two frozen Pydantic models exported from `khora` and `khora.filter`. `FilterPushdownReport` carries the top-level `pushed_down` / `post_filtered` flags, the `pushed_keys` / `post_filtered_keys` / `unenforced_keys` constraint-leaf partition, and a per-channel `channels: dict[str, FilterChannelReport]` breakdown. Built backend-agnostically from per-channel pushdown facts so every engine reports the same honest schema. A per-call `filter_plan_out` sink (replacing the per-store `_last_filter_plan` attribute) means concurrent recalls on a shared store can no longer clobber each other's report.
- **`unenforced_keys` makes the constraint-leaf partition total and disjoint** (#1217): `pushed_keys` / `post_filtered_keys` / `unenforced_keys` now partition *every* filter constraint leaf into exactly one bucket — pushed into the backend query, re-checked in memory, or gated by no channel at all. The field is additive (defaults to `[]`, so existing readers are unaffected) and is sorted/JSON-stable like its siblings. A correct recall enforces every leaf, so `unenforced_keys == []`; a non-empty value is the in-band signal that a result-producing path returned candidates without enforcing those leaves.
- **VectorCypher now emits `engine_info["filter"]`** (#1181): every `recall()` on the vectorcypher engine carries the honest per-channel report, built from each retrieval channel's actual backend compile on the path it took. The vector and BM25 channels reflect their backend's real pushdown — full on the raise-mode Postgres/SurrealDB stores, a pushed/post-filtered split on the embedded `sqlite_lance` store. The graph channel reports the keys pushed into the Cypher `WHERE` taken from the compile that actually executed inside the graph chunk fetch (threaded back out, not re-derived from a parallel probe), so it credits a pushdown only on the Neo4j traversal path that ran one — the PageRank and embedded-graph paths push nothing and re-check in memory — and always records a defensive full-predicate in-memory re-check. A channel that does not execute under the filter is omitted; a no-filter recall yields an all-`False` report.
- **Cross-engine filter-report invariant gate** (#1185, #1217): a CI gate asserts that every engine's emitted `engine_info["filter"]` obeys the engine-independent invariants — the pushed / post-filtered / unenforced key lists totally and disjointly partition the filter's constraint leaves, `unenforced_keys == []`, `pushed_down` derives from the list partition (not the `post_filtered` flag, which a defensive in-memory re-check sets even at full pushdown), every channel addresses only real leaves with sorted lists, and a no-filter / `filter={}` recall is the canonical empty carrier. It runs once per engine configuration, riding the end-to-end recall matrix (where the report is actually emitted). A hermetic drift guard keeps the gate's lane set in lockstep with the e2e matrix so a new engine config cannot ship without invariant coverage.
- **Recall-filter compilers for SQLite, SurrealDB, and Weaviate backends** (#1045, #1042, #1040): the deterministic recall-filter AST now compiles to native pushdown on three more backends. `compile_lance` lowers to a SQLite `WHERE` with `json_extract` / `json_type` / `json_each` metadata pushdown (superset-safe split mode, gated by a connect-time `json_valid` probe). `compile_surrealdb` emits a SurrealQL predicate with native nested dot-path descent (`metadata_.a.b.c`), `type::is::*` runtime type gates, and array-aware equality/membership. `compile_weaviate` lowers the pushable slice onto the Weaviate v4 Filter API (declared properties only, monotone-narrowing positives) with the Python predicate compiler enforcing the whole filter as a post-filter. All three honor every filter without raising on undeclared/metadata predicates.
- **Recall-filter conformance corpus across fourteen filter families and seven engine configs** (#1044, #1049, #1058, #1073, #1083): a hand-authored conformance catalog (F-OP, F-SUGAR, F-IMPOSSIBLE, F-EXISTS, F-COERCE, F-POLARITY, F-OBJEQ, F-DOTKEY, F-ARRAY, F-LOGIC, F-DATES, F-NULLVAL, F-SEL, F-UNSUP) with hand-authored `expected_ids` cross-checked by a Python oracle. Every family runs through all five live-DB backend compilers (postgres / cypher / surrealdb / weaviate / sqlite_lance) plus the python/chronicle in-memory legs, each asserting row-set equality against the oracle. Ships as a parallel per-backend CI job (`filter-conformance.yml`) plus a dedicated slow/e2e `remember()→recall(filter=)` row-set matrix (`e2e.yml`) across seven engine configurations, with fail-loud tripwires for the container-free lanes (#1094) and a verification-coverage meta-gate (#1091) that makes a "silently disabled" filter test impossible to merge.
- **VectorCypher recall-filter pushdown** (#1046, #1051): the validated filter AST is now threaded through every VectorCypher channel and adaptive sub-search — vector, BM25, the Cypher graph channel (with `query.metadata_overfetch_multiplier` over-fetch + in-memory post-filter for residual metadata leaves), and the change-decomposition / recency sub-searches. `RecallFilterUnsupportedError` from `compile_cypher` surfaces to the caller instead of degrading to vector-only; a white-box pushdown-spy suite (#1071) proves the filter reaches every channel with a matching `canonical_hash`.
- **VectorCypher recency channel fires again** (#1186): the temporal store now implements `search_recent_chunks` (the channel had been silently dead since the temporal-store rewiring). The pgvector temporal store sorts `khora_chunks` by `COALESCE(occurred_at, source_timestamp, created_at) DESC` (served by a new `ix_khora_chunks_ns_recency` expression index), preserves the embedding so the caller's cosine relevance gate runs, and backends without the capability return the protocol default `[]`. Embedded (`sqlite_lance`) support is tracked in #1182.
- **Distinct `occurred_at` event-time persisted and read on every chunk backend** (#1054, #1062, #1075): pgvector, embedded sqlite_lance (new dialect-portable migration), and SurrealDB chunk write/read paths now persist and read back `occurred_at` instead of dropping it and silently falling back to `source_timestamp` via `COALESCE`. SurrealDB also now persists `last_accessed_at`.
- **SurrealDB recall reinforcement** (#1090): `update_last_accessed` implemented on the SurrealDB vector adapter (matching pgvector / sqlite_lance), so SurrealDB is now a supported backend for Chronicle reinforcement-on-recall.
- **Public-surface snapshot + cold-import guardrail tests**: a pinned snapshot of `khora.__all__` (now including `FilterPushdownReport` / `FilterChannelReport`) and a guard asserting `import khora` no longer eagerly loads `khora.query.engine` or `khora.integrations`.
...
- **Deprecated `start_time` / `end_time` recall bounds now forward only the recency-window `temporal_filter`** (#1084): they previously also folded into an `occurred_at` event-time filter, which on Chronicle false-emptied anchor-less chunks. The public `occurred_at` filter keeps its documented event-time semantics (no `created_at` fallback).
- **Checksum dedup is now scoped by `external_id` and `session_id`** (#1139): `remember()` deduped on content checksum alone (namespace-scoped), so a new `external_id` / `session_id` carrying previously-seen content hit the dedup and silently vanished — and worse, session B's content resolved to session A's document, so `forget_session(A)` could delete B's only copy. Dedup now applies only when every caller-supplied identity also matches; callers supplying neither keep the checksum-only behavior.
- **QueryCache key now folds in the temporal filter and result-shaping config** (#1129): the cache keyed only on `(query, namespace_id, mode)`, so within the TTL a recall with a `temporal_filter` (or a config overriding `max_chunks`, thresholds, reranking/hyde flags) served the cached result computed under different parameters.
- **Filter canonical hash distinguishes exact-array from containment operands** (#1072): a bare-list `$eq` (exact-array equality) and an explicit `{"$eq": [list]}` (array containment) canonicalized to the same JSON array and collided on `canonical_hash` despite compiling to different row sets — a latent recall-filter cache hazard. The operand's container kind is now a forge-proof structural field on the clause record.
- **Recall-filter metadata-leaf observability moved to a recall-span attribute** (#1100): the experimental `khora.recall.filter.unindexed_metadata` counter (which over-counted by an engine-dependent multiplier across independently-compiling channels) is removed in favor of an exact-by-construction `filter.metadata_leaf_count` attribute set once per recall on the `khora.recall` span.
- **README and docs overhaul**: README reframed around a knowledge-repository identity with a runnable above-the-fold quickstart, `uv add` install commands, `db-up`/`db-down` Makefile aliases, and documentation links pointed at docs.deyta.ai/khora. A two-dimensional docs pass softened harsh phrasings (load-bearing warnings preserved) and corrected stale claims (dream-phase op counts, query/temporal defaults, API surface, rust-acceleration details).

### Fixed

- **Bare-list `$eq` exact-array recall no longer crashes on `sqlite_lance` when a scalar is stored at the path**: the exact-array equality emitter ran the inner `json()` parse over the stored node unconditionally, so a metadata path holding a scalar string (rather than an array) raised a SQLite JSON error and aborted the recall. The comparison is now `CASE`-gated so the inner `json()` runs only when the node is a JSON array; a scalar (or absent) node reads false via `coalesce` instead of erroring.
- **Timezone normalization across temporal sort/compare boundaries** (#1061, #1115, #1145, #1146, #1141): mixed tz-naive (embedded SQLite) vs tz-aware datetimes raised `TypeError` / "can't compare offset-naive and offset-aware" and crashed recalls instead of narrowing. Normalized to UTC at the recall-filter system-key comparison boundary (#1061), in the VectorCypher `_simple_retrieve` sort key (#1115), before `max()` in chronicle temporal decay / recency-multiplier (#1145), in `gc.expire_sessions` against the DB-derived `latest` timestamp (#1141), and sqlite_lance timestamps are now stored/compared in canonical UTC (#1146).
- **Rust/NumPy acceleration parity** (#1130, #1131, #1132, #1133): the Rust kernels diverged from their Python fallbacks in ways that change persisted data or ranking. `batch_recency_scores` now clamps `age_days` to 0 so a forward-dated chunk gets full freshness instead of an unbounded multiplier (#1130); `detect_communities` breaks ties on the smallest community id for reproducibility across processes (#1131); `batch_cosine_similarity` / `batch_dot_product` now raise `ValueError` on a query/candidate dimension mismatch like NumPy instead of silently scoring a truncated prefix (#1132); and `normalize_entity_name` converged onto the less-aggressive Python punctuation trim so the Rust wheel can never collapse distinct entity names into one row (#1133).
- **Multi-backend graph write integrity on re-ingest** (#1149, #1150, #1151, #1057, #1066): batch entity upserts now honor the #806 id-remap contract on every backend, so relationship endpoints built from extraction-time ids resolve after the upsert. Neptune and AGE gained default `GraphBackendBase` batch implementations (previously their entities went only to the pgvector mirror and relationships were dropped) (#1149); Memgraph (#1150) and SurrealDB (#1151) now write the stored id back into the caller's `Entity` (relationships touching previously-known entities were being silently lost on re-ingest); sqlite_lance graph-only stacks now persist entity embeddings through the graph upsert (#1057); and chronicle relationships now persist + count on the graph-less PG path via a pgvector fallback (#1066).
- **pgvector entity upsert accumulates source provenance instead of overwriting it** (#1039, #1048): on re-ingest the upsert replaced `source_document_ids` / `source_chunk_ids` with only the newest write's ids, making `forget()` misclassify multi-sourced survivor entities. The upsert now unions and de-duplicates ids with per-column retention caps (documents 100, chunks 250) matching Neo4j's `ON MATCH` semantics.
- **Neo4j graph-layer correctness bugs surfaced by enabling Neo4j in CI** (#1043, #1153, #1154): restored the source-document-id remap guard (no longer appends ids unconditionally), coerce stored ISO-string `valid_until` via Cypher `datetime()` before comparing to `now()` (the string-vs-datetime compare silently dropped future-dated edges), strip the replaced doc id from co-sourced survivors on replace, scope the relationship-source strip to its namespace, query serialized-JSON attributes correctly via `CONTAINS` prefilter + Python exact-match (#1153), and bound `get_neighborhood` inside the projection so `LIMIT` after `collect()` no longer no-ops (#1154).
- **Memory facts cascade through chronicle forget paths** (#1140): `memory_facts` has no FK to chunks, so forgotten content's facts stayed `is_active=True` with dangling `source_chunk_ids` and kept feeding reconciliation — breaking the `forget_session` contract. A new `delete_facts_for_chunks` primitive on all three fact-storing backends hard-deletes any fact referencing a deleted chunk, with ADR-001 graceful degradation on cleanup failure.
- **SurrealDB `option<array>` element typing so provenance persists** (#1168): episode arrays (`entity_ids`, `source_document_ids`, `source_chunk_ids`) were declared bare `option<array>`, which SCHEMAFULL SurrealDB silently coerces to `[]`, so episode provenance never persisted. Now typed `option<array<string>>`, following the `memory_fact.source_chunk_ids` fix.
- **LLM-extracted `valid_from` / `valid_until` and `source_timestamp` preserved on extraction and batch ingest** (#994, #992, #1121): extraction wrote `valid_from = chunk.created_at` (an ingest-time value) and discarded the LLM's real-world temporal signal, collapsing every dated fact to "now" — now resolved same-axis (LLM signal → `source_timestamp` floor → None) (#994). The VectorCypher streaming batch path and orphan-recovery path now consult per-doc / batch `source_timestamp` and persisted `metadata["occurred_at"]` instead of stamping ingest time (#992, #1121).
- **Chronicle entity-channel relevance gate repaired** (#1143, #1157): the gate had been dead since it was written — `batch_cosine_similarity` returns sorted `(index, score)` pairs but the channel positionally `float()`-ed each element, raising `TypeError` that a bare `except` swallowed by forcing every similarity to `1.0`, so entity-adjacent but semantically irrelevant chunks flowed into RRF fusion at full score. Now maps each `(local_idx, score)` back to its chunk and seeds every chunk at `0.0` so dropped chunks stay gated out.
- **Cross-session expansion re-sorts before trim and records degradations** (#1118, #1119): expansion chunks with discounted scores were appended without re-sorting, so the positional `[:limit]` trim cut higher-scored expansion chunks and violated the ranking contract (#1118); three bare `except Exception` blocks now record ADR-001 degradations instead of silently returning the un-expanded list (#1119).
- **`$not` over a multi-operator expression lowers to `NOT(AND(...))` preserving all predicates** (#1127): a field-position `$not` over two-or-more operators built a multi-child `NOT` (violating the AST one-child invariant), and every backend compiler negated only the first child, silently dropping the rest and diverging from Mongo. Lowering now wraps multiple clauses in an `AND` under a single-child `NOT`.
- **Metadata object filters use exact JSONB equality, not containment** (#1047, #1055, #1035): a metadata sub-path dict operand and dict `$in`/`$nin` elements were routed through `@>` containment, so a stored superset object wrongly matched; now exact `#> = '{...}'::jsonb` object-equality. Separately, scalar `$eq`/`$in` now also matches array-valued metadata fields via an OR'd array-wrapped containment form (#1035).
- **Embedded VectorCypher graph-channel recall no longer returns zero chunks** (#1104, #1086): three independent defects — `_cypher_expand` calling `get_neighborhoods_batch` without `namespace_id` (raising `TypeError`), the normalization loop dropping `Entity` domain objects it only accepted as dicts, and `get_chunks_batch` querying only `chunks` instead of falling back to `khora_chunks` — are fixed, with a structured degradation + `khora.vectorcypher.cypher_expand.degraded_total` counter for genuinely unrecognized shapes.
- **Embedded BM25 channel enforces the recall filter** (#1080, #1098): `search_fulltext` on the sqlite_lance and SurrealDB temporal stores accepted `filter_ast` for protocol parity but did not forward it into the BM25 query, letting filter-excluded chunks leak into RRF fusion. Both now compile and AND the filter into the keyword channel.
- **Superset-safe temporal pre-filter bounds on the embedded stores** (#1095, #1110, #1136): the sqlite_lance LanceDB coarse vector pre-filter used a strict `<` upper bound (false-excluding a chunk exactly on the bound instant before the inclusive SQLite refinement ran) — now inclusive `<=` (#1095, #1110); and the LanceDB pre-filter narrowed on `created_at` while the contract axis is `COALESCE(source_timestamp, created_at)`, dropping backfilled chunks before the refinement could admit them — the `created_at` pre-filter is removed (#1136).
- **SurrealDB / Cypher system-key filter semantics aligned with the oracle** (#1073): system date range ops on a nullable SurrealDB column are guarded with `IS NOT NONE` (SurrealQL `NONE < x` is TRUE), system-key `$exists` compiles to a constant on Cypher and SurrealDB (always-present axiom), and Cypher / sqlite_lance defer an `$or`/`$not` wrapping an unpushable metadata leaf to the post-filter instead of false-excluding every row. Postgres `$in`/`$nin` dict elements are now array-aware exact-per-element object equality.
- **Honest filter-pushdown reporting on the skeleton engine** (#1069): `pushed_down` was derived from a hardcoded `backend == "pgvector"` check, reporting `False` on `sqlite_lance` even at full pushdown. It is now derived from what each channel's compiler actually consumed (`pushed_keys`) versus re-checked in memory (`post_filtered_keys`); a defensive full-predicate re-check sets `post_filtered=True` without demoting a fully-pushed leaf.
- **VectorCypher typed-entity-recent fast path no longer bypasses the recall filter** (#1183): the fast path cannot enforce caller filters (chunk metadata is a serialized JSON property on the graph node), so filtered recalls now fall through to the full retrieval path; unfiltered recalls keep the single-Cypher fast path.
- **Recall filter carried through the transient vector-only fallback** (#1079): a recall that hit a transient Neo4j error degraded to vector-only *without* the filter, returning filter-violating chunks. `filter_ast` is now threaded into `_vector_only_fallback`.
- **Weaviate `KhoraChunk` collection stores the eight denormalized document fields** (#1180): recall filters over `source_type` / `source_name` / `source_url` / `external_id` / `content_type` / `source` / `title` / `source_timestamp` previously matched zero rows because the fields weren't persisted; the seven string keys are stored as filterable-only TEXT (kept out of the BM25 corpus) and `source_timestamp` as DATE.
- **LLM calls bounded by a wall-clock timeout; extraction cannot hang** (#1059, #1109, #1105): litellm's default transport builds no `total` ClientTimeout, so a TLS-handshaked-then-silent connection hung indefinitely and deadlocked large-corpus builds — every async litellm call (embedding, extraction, chat, chronicle event/fact/contradiction) is now wrapped in `asyncio.wait_for` (#1059). Entity extraction additionally bounds the bisection fan-out and shared-semaphore acquire, excludes `CancelledError`/`TimeoutError` from the retry predicate, and marks timed-out documents failed instead of producing silently-empty output (#1109). Deterministic credential/auth errors now fail fast instead of burning 3× retries + backoff (#1105).
- **Level-2 batch task strongly referenced; awaiters never stranded** (#1161): the Level-2 evaluator fired `loop.create_task` and dropped the handle (GC-collectible mid-flight), and an exception before resolving per-item futures blocked every caller forever. The handle is now held with an `add_done_callback`, and an escaped exception fail-open-resolves every pending future.
- **`Khora.shared()` is loop-safe** (#1160): the module-level shared lock bound to the first event loop and the cache handed back a Khora whose asyncpg pool lived on a now-dead loop, raising under a second loop (sequential `asyncio.run`, pytest-asyncio per-test loops). The lock is created lazily per running loop and each cached instance stores its loop, dropping and rebuilding on a mismatch (mirroring the #790 fork handler).
- **Shared-state race in per-call `hybrid_alpha` / `SearchMode.ALL`** (#1116, #1117, #1156): `recall()` mutated the *shared* retriever config and restored it in `finally`, so overlapping recalls observed the wrong blend factor — the effective alpha is now threaded as an explicit `hybrid_alpha_override` and the shared config is never written (#1116). Also forwards caller provenance on the `external_id` race-retry path (#1117) and adds the missing `recency_bias` param to `VectorCypherEngine.recall` for protocol conformance (#1156).
- **Namespace-scoped, content-stable entity cache key; reranker loaded off the event loop** (#1159, #1162): `HybridQueryEngine` keyed its dedup cache by `id(query_embedding)` (a recyclable Python `list` id), so on a shared engine query B could await query A's cached task and get A's results — for A's namespace, a silent cross-namespace correctness bug. Now keyed on `(namespace_id, sha1(embedding bytes))` with self-evicting entries (#1159). `CrossEncoderReranker` model construction now runs via `asyncio.to_thread` under a lock instead of blocking the event loop for seconds of torch weight loading (#1162).
- **Extraction / expansion robustness** (#1122, #1123, #1124, #1125, #1126): hoisted telemetry imports so a `cross_tool_unification:false, relationship_inference:true` config no longer raises `UnboundLocalError` and FAILs every document (#1122); dropped batch-extraction sections now carry an `"error"` so they flow into the #889 diagnostics instead of becoming silent empty successes (#1123); explicit `enable_inference` / `enable_unification` kwargs now win over expertise values (#1124); `select_for_extraction` now applies `min_score` as a genuine floor (`AND` not `OR`) so KET-RAG cost saving is no longer a no-op (#1125); and `extends` config merge preserves an explicitly-disabled `expansion` (#1126).
- **Invalidated canonicals excluded from event clustering; abstention-drift direction corrected** (#1147, #1148): the dream event-clustering planner re-admitted tombstoned `chronicle_events` rows, so a live event could be soft-merged into an already-invalidated row (#1147); and the abstention-drift "raise threshold" rationale was inverted relative to the `0.0→1.0` combined-score direction (#1148).
- **Neptune IAM auth re-signed per connection** (#1152): `connect()` installed a single frozen SigV4 signature for the driver's lifetime; since signatures expire after ~5 minutes, every pool connection opened after that window was rejected and a long-running process lost graph connectivity. A bearer auth manager now re-signs (and re-freezes credentials from the boto3 chain) on every token refresh.
- **Embedded `prefer_current` traversal honors expired relationships** (#1096): the embedded graph backend's relationship writers were INSERT-only, so re-saving an edge to close its validity window raised on the PK conflict and the expiry never persisted. Both writers now UPSERT the mutable/temporal columns (leaving id, endpoints, provenance, and reserved bi-temporal columns untouched).
- **sqlite_lance `replace_document_extraction` is transaction-safe** (#1134, #1135): the coordinator passed `session=txn.session` to the vector backend unconditionally, crashing on sqlite_lance (`create_chunks_batch` has no `session` kwarg) and leaving a DELETE pending in aiosqlite's implicit transaction with the LanceDB compensation never running. The coordinator now passes `session=` only to backends in the SQLAlchemy session, and the sqlite_lance adapter always commits its own work and runs the compensation.
- **Alembic version-stamping is atomic per migration** (#1053): back-to-back full-chain replays could fail stamping ("0 found") because autocommit `CONCURRENTLY` migrations committed the outer transaction mid-chain. `transaction_per_migration=True` plus a session-scoped `pg_advisory_lock` (released in a `finally`) make each migration commit its DDL and stamp atomically. SQLite is unaffected.
- **`gc.expire_sessions` paginates all namespaces** (#1142): enumeration fetched a single page (limit 1000), so deployments with >1000 active namespaces silently never expired sessions past the first page. It now loops over every page until exhausted.
- **Dual-write partial-failure observability + native Neo4j `count_entities`** (#1138, #1155): `create_entity` / `update_entity` dual-writes now sequence vector-first and increment a `khora.storage.{create,update}_entity.partial_failure` counter when the graph write fails after the vector commit (#1138); Neo4j gained a native `count_entities` (`RETURN count(e)`) instead of materializing up to 100k rows and capping at 100,000 on every `stats()` call (#1155).
- **halfvec HNSW indexes recreated on drop/recreate round-trips** (#1137): `drop_hnsw_indexes` dropped the halfvec HNSW indexes but nothing recreated them, so after one bulk-load cycle halfvec-cast searches sequential-scanned. A new `ensure_halfvec_indexes()` is called from `optimize_postgresql` alongside `ensure_hnsw_indexes`.
- **Filter-conformance corpus and seeder accuracy fixes** (#1164, #1184, #1216, #1076, #1089): the conformance runner now denormalizes the seven string document keys onto chunk rows (#1164); `source_type` defaults to a non-null `"library"` matching production (#1184); `created_at` is modeled as always-present (NOT NULL + writer-stamped) instead of authoring an unreachable absent state (#1216); the Neo4j conformance leg seeds via the production graph write path (#1076); and stale created_at classification docs were corrected (#1089).
- **Test-infrastructure and CI hardening**: self-heal the venv on a CI cache miss under `UV_NO_SYNC=1` and make `examples-smoke` hermetic (offline reranker) (#1178); pin a `logfire` / `opentelemetry-sdk` pair per resolver fork so a bare `uv run pytest` no longer crashes at plugin load with `ImportError: LogData` (#1163); reset OpenTelemetry providers after the OTLP smoke test to prevent a cross-test tracer leak that masked real errors (#1107); run the hermetic cross-namespace IDOR signature gate in the CI unit job (#1108); and stub entity extraction in chronicle recall-filter tests to drop ~585s of live-LLM retry backoff to ~8s (#1106).
...
- **Embedded VectorCypher graph-channel recall no longer silently returns zero chunks** (#1086): on the embedded SQLite + LanceDB stack the graph channel produced no chunks because of three independent defects. (1) `_cypher_expand` called the storage coordinator's `get_neighborhoods_batch(...)` without the required `namespace_id` keyword, raising a `TypeError` that collapsed the graph-expansion step before the recursive CTE ran. (2) The embedded backend returns `Entity` domain objects from `get_neighborhoods_batch`, but the `_cypher_expand` normalization loop only accepted `dict` entries, so every expanded entity was dropped; the loop now maps `Entity` objects to the dict shape the scoring loop reads and records a structured `Degradation` (`component="vectorcypher.cypher_expand"`, `reason="unrecognized_neighborhood_shape"`) on `RecallResult.engine_info["degradations"]` for any genuinely unrecognized entry shape. (3) `SQLiteLanceVectorAdapter.get_chunks_batch` queried only the `chunks` table, but the temporal engines write to `khora_chunks` (#905), so the graph-channel chunk ids resolved to nothing; it now falls back to `khora_chunks` for the unresolved ids, swallowing the missing-table error on a chronicle-only stack the same way `_get_chunks_by_document_temporal` does. The public `recall()` signature is unchanged.
- **Embedded BM25 channel now enforces the recall filter.** `SQLiteLanceTemporalStore.search_fulltext` accepted `filter_ast` for protocol parity but did not forward it into `_bm25_search`, so a `recall(filter=...)` over the embedded SQLite+LanceDB stack let filter-excluded chunks leak through the BM25/keyword channel (the pgvector sibling already enforced). `search_fulltext` now forwards `filter_ast=filter_ast`, so `_bm25_search` pushes the compilable leaves into SQL and re-checks the rest against the decoded chunk via the `compile_python` post-filter — matching the vector channel and the pgvector sibling.
- **SurrealDB system date range ops now exclude an absent (NONE) row.** SurrealQL's NONE comparison is asymmetric — `NONE < x` / `NONE <= x` evaluate to TRUE while `NONE > x` / `NONE >= x` are FALSE — so a lower-bound (`$lt`/`$lte`) range op on a nullable `option<datetime>` system column (`occurred_at` / `created_at` / `source_timestamp`) wrongly included rows whose date is absent, diverging from the oracle (a missing field never satisfies a range comparison). `compile_surrealdb` now guards every system-key range op with `field IS NOT NONE AND field <op> bound`, so an absent value never matches; a wrapping `$not` still flips the absent row back in (Rule 2 parity). No-op for `$gt`/`$gte` (already FALSE on NONE), the fix for `$lt`/`$lte`.
- **Cypher and SurrealDB `$exists` on a SYSTEM key is now a constant, matching the always-present axiom.** The Postgres and SQLite (lance) compilers and the Python oracle treat a system key as always-present, so `$exists:true` is a constant TRUE (every row) and `$exists:false` a constant FALSE. The Cypher (`var.key IS NOT NULL`) and SurrealDB (`field IS NOT NONE`) compilers instead emitted a *presence* test, which diverges whenever the denormalized document key is genuinely unset on the node/record (the unwritten doc keys read NULL/NONE) — `$exists:true` wrongly excluded those rows. Both now emit the constant `true` / `false` for a system-key `$exists`, aligning all five backends with the oracle. Metadata-path `$exists` is unchanged (real presence on the JSON path).
- **Postgres `$in`/`$nin` dict element is now array-aware exact-per-element object equality.** A dict element of a metadata `$in`/`$nin` previously compared only the whole extracted node to the operand object, so an array field that *held* the exact subdocument (e.g. `metadata.labels = [{"team": "ingest"}]` against `$in: [{"team": "ingest"}]`) was wrongly excluded. The compiler now emits `node = '{...}'::jsonb OR (jsonb_typeof(node) = 'array' AND EXISTS(SELECT 1 FROM jsonb_array_elements(node) e WHERE e = '{...}'::jsonb))` (coalesced total) — matching the Python oracle's `_md_contains` for a dict value (an array element OR the scalar node exactly equal to the dict). Stays exact `=` per element (a stored superset object still does NOT match), never `@>` containment.
- **sqlite_lance defers a dict / `None` `$in`/`$nin` element to the post-filter instead of mis-pushing it.** A dict element of a metadata `$in`/`$nin` was bound as a JSON-text scalar in the `json_each` containment (could neither represent the object nor match it array-aware), and a `None` (JSON-null) member was excluded by the containment's `je.type` gate — yet both were marked consumed, silently diverging from the oracle. Such a leaf (and an `$or`/`$not` wrapping it) is now left unconsumed so the `compile_python` post-filter re-checks it (Rule 2: a documented capability gap, not a silent drop).
- **Cypher defers an `$or`/`$not` wrapping an unpushable metadata leaf to the post-filter instead of false-excluding.** Cypher pushes only system keys; a metadata-path leaf emits the non-constraining `true` under split mode. But `$or`/`$not` were composed naively, so `$not` over an unpushable metadata leaf (e.g. `{metadata.k: {$not: {$exists: true}}}` or a `$not`-range over `metadata.num`) compiled to `(NOT (true))` ≡ `false`, which excluded every row — and the `compile_python` post-filter only narrows, so it could not recover the wrongly-dropped rows, diverging from the oracle and every other backend. The compiler now defers the whole `$or`/`$not` node to the non-constraining `true` (consuming nothing) whenever any descendant leaf is unpushable, keeping the prefilter a superset; `$and` still distributes, so a positive system-key sibling alongside a deferred metadata clause still pushes its constraint. Matches the all-or-nothing guard already in the sqlite_lance compiler.
- Postgres recall-filter compiler binds JSONB path operands as `text[]` so path filters match correctly (#1026).
- **pgvector entity upsert now accumulates source provenance instead of overwriting it** (#1039): on re-ingest, the pgvector entity upsert replaced `source_document_ids` / `source_chunk_ids` with only the newest write's ids instead of merging them. On a PostgreSQL + Neo4j stack this made `forget()` misclassify multi-sourced survivor entities and fail to strip the forgotten document's id. The upsert now unions the existing and incoming ids, de-duplicates them, and keeps the most-recent distinct ids (newest last) capped per column - documents at 100 and chunks at 250, mirroring Neo4j's `entity_source_document_ids_max` / `entity_source_chunk_ids_max` - matching Neo4j's `ON MATCH` semantics, so `forget()` strips provenance correctly across both backends.
...
- **`khora[embedded]` now installs SQLite + LanceDB instead of SurrealDB** (#966): the `embedded` extra is now an alias for `khora[sqlite-lance]` (`aiosqlite`, `lancedb`, `pyarrow`), matching the zero-infrastructure embedded stack documented and tested across the codebase (VectorCypher, Skeleton, Chronicle). Installers that pinned `khora[embedded]` expecting the SurrealDB SDK should switch to `khora[surrealdb]`; the `surrealdb` extra is unchanged.

### Added

- **Public `khora.pipelines.extract_source_timestamp(metadata)` helper** (#969): the timestamp-extraction logic shared by the ingestion pipeline is now an importable public function.

### Fixed

- **Memgraph adapter hardening** (#919, #920, #921, #922): four divergences from the Neo4j reference. Ingest no longer silently writes zero entities/relationships on `backend=memgraph` (`upsert_entities_batch` / `create_relationships_batch` added as pure-Cypher `UNWIND` + `MERGE`, no APOC); `count_relationships` is implemented instead of raising `NotImplementedError`; `create_relationship` uses `MERGE` so re-asserting an edge is idempotent instead of duplicating it; and `get_neighborhood` now returns `relationship_type` (read from the edge label) to match Neo4j's output shape.
- **LLM `model_list` / `router_settings` config is now applied** (#936): completions route through a cached `litellm.Router` when a `model_list` is configured, so multi-provider fallback works instead of being a silent no-op. Stacks without a `model_list` keep the direct `litellm.acompletion` path (default behavior unchanged); `_telemetry_op` instrumentation is preserved on both paths.
- **Telemetry events survive a transient flush failure** (#924): `TelemetryCollector._flush` now re-enqueues the drained batch at the front of the buffer on a failed insert (deadlock, statement timeout, dropped connection) so the next tick retries it, instead of discarding the in-flight batch and only logging a warning. The buffer is capped at `max_buffer_size` (default 10000); on sustained-failure overflow the oldest events are dropped with a WARNING and the new `khora.telemetry.flush.dropped_total` counter increments.
- **Staging-queue memory is now bounded** (#932): the pending-processor queue (`Khora._processor_queue`) is unbounded and previously held each document's full content in RAM, so a large batch - or orphan recovery sweeping stale PENDING rows across all namespaces on restart - could materialize the whole backlog's content at once. Queued items now carry only `doc_id` / `namespace_id` / batch registration (~100 bytes) and the worker re-loads the full document from storage at dequeue, so peak content RAM is bounded by the number of concurrently-draining workers. A row forgotten between enqueue and dequeue is handled as an ADR-001 skip (INFO log, `khora.documents.processor.degraded_total` counter, batch registration decremented so `BatchHandle.wait()` can't hang).
- **Semantic-hook Hamming pre-filter handles mixed embedding dimensions** (#927): the bit count is now stored per filter instead of a single per-instance value that the last `register_filter` clobbered, and a cross-dimension comparison is rejected (WARNING, no bogus score) instead of silently zip-truncating mismatched-length codes and skewing toward false rejection.
- **rapidfuzz fallback uses Jaro-Winkler** to match the calibrated entity-resolution thresholds (ingest merge 0.85, link 0.8) that the Rust backend already uses. The fallback previously used `fuzz.ratio` (Indel/LCS), which scored prefix-overlapping name variants ~0.1 lower (e.g. "harry potter" vs "harry james potter" scored 0.80, below the 0.85 merge threshold), so without the Rust extension entity variants stayed unmerged and the graph fragmented.
- **Silent `source_timestamp` loss for non-snake_case connector keys** (#969): the ingestion extractor only matched exact snake_case keys (`occurred_at`, `sent_at`, …), so connectors emitting camelCase / kebab-case / SCREAMING_CASE / TitleCase variants (`occurredAt`, `occurred-at`, `OCCURRED_AT`, `OccurredAt`) silently fell back to ingest time, breaking recency scoring. Key matching is now case/separator-insensitive; an exact key still wins over a normalized variant when both are present. `updated_at` is now honored by the extractor (appended last in priority for both event and non-event sources).
- **Documentation corrections** (#937, #938, #939, #940): `retrieval-tuning.md` `diversity_lambda` default corrected to `0.5` (was documented as `0.7`); `configuration.md` `KHORA_QUERY_RECENCY_WEIGHT` default corrected to `0.35` (was `0.2`); the fabricated 4-tier adaptive top-k table in `retrieval-tuning.md` replaced with the engine's actual two-branch reduction behavior (`very_focused` < 0.3 and `single_topic` < 0.5, both only ever lowering the limit); and the `fusion.md` / `search-modes.md` examples now read per-method data from `result.engine_info['search_methods']['by_method']` instead of the non-existent `result.search_contributions`.
