...
- **Cross-engine filter-report invariant gate**: a CI gate asserts that every engine's emitted `RecallResult.engine_info["filter"]` obeys the engine-independent `FilterPushdownReport` invariants — the pushed / post-filtered key lists totally and disjointly partition the filter's constraint leaves, `pushed_down` derives from the list partition (not the `post_filtered` flag, which a defensive in-memory re-check sets even at full pushdown), every channel addresses only real leaves with sorted lists, and a no-filter / `filter={}` recall is the canonical empty carrier. It runs once per engine configuration. Because `engine_info["filter"]` is emitted only on a real recall, the gate rides the end-to-end recall matrix (one engine config per leg) rather than the compile-level conformance matrix, which never runs a recall and emits no report. A hermetic drift guard keeps the gate's lane set in lockstep with the e2e matrix so a new engine config cannot ship without invariant coverage.
...
- **`FilterPushdownReport` now carries an `unenforced_keys` list, making the constraint-leaf partition total and disjoint.** The top-level `pushed_keys` / `post_filtered_keys` / `unenforced_keys` lists now partition *every* filter constraint leaf into exactly one bucket: pushed into the backend query, re-checked in memory, or gated by no channel at all (enforced by nothing). The field is additive (defaults to `[]`, so existing readers are unaffected) and is sorted/JSON-stable like its siblings. A correct recall enforces every leaf, so `unenforced_keys == []`; the cross-engine filter-report invariant gate asserts exactly this per engine configuration, turning a previously-silent "leaf the filter constrains but no channel enforces" defect into a hard failure.
...
- **Chronicle emits the canonical `FilterPushdownReport`**: `RecallResult.engine_info["filter"]` on the Chronicle engine now carries the full report shape (`pushed_keys` / `post_filtered_keys` plus the per-channel `channels` breakdown, single `chunks` channel) instead of the hand-rolled `{pushed_down, post_filtered}` two-field dict. **Behavior change:** `pushed_down` now uses ALL-leaves semantics (was "any key pushed"). Chronicle pushes only the `source_timestamp` date bound into the recency window while always enforcing the full predicate in memory, so a multi-predicate filter now reports `pushed_down=false` with the pushed/post-filtered keys split out — exposing the partial-pushdown truth the old boolean hid. This is the released metric of record for callers reading the filter report. Callers that read `pushed_down` as a proxy for "was any filter work pushed down" should read a non-empty `pushed_keys` instead.
