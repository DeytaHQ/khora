...
- **SurrealDB forget-cascade caveat (#923, 0.18.0):** the forget cascade keys off `source_document_ids` refcounting. SurrealDB entities/relationships ingested BEFORE 0.18.0 carry an empty `source_document_ids` (the persistence bug #923 fixes) and will NOT be cleaned by `forget()` - neither stripped nor orphan-deleted. Only records ingested on >=0.18.0 participate. No automatic backfill; re-ingest affected documents on >=0.18.0 to clean them. See `docs/architecture/storage-backends.md#forget-cascade-and-source_document_ids-refcounting-923`.
