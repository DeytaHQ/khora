...
- **Logfire is optional** — `_HAS_LOGFIRE` flag in `telemetry/logfire_integration.py` controls OTEL span emission. Install with `pip install khora[logfire]`. When absent, `logfire_span()` yields `None` (zero-cost no-op). Custom telemetry (`collector.record_*`) fires regardless of logfire presence. Khora never calls `logfire.configure()` or `logfire.instrument_*()` — that's the consumer's responsibility
