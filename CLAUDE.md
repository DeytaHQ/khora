...
- **Citation sources** — `Source` dataclass in `core/models/source.py` carries document provenance (document_id, title, url, source_type, source_tool). `Chunk.source` holds a resolved `Source` for each chunk. `Entity.source_documents` lists resolved sources for all documents that contributed to an entity. `Entity.source_tool` stores the canonical SaaS tool name (e.g. "slack", "linear"). Pass `source_tool` to `remember()` and `remember_batch()` to tag ingested content. Source types are extensible via `register_source_type()` and `register_source_alias()`
