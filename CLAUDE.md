...
- **Never use `create_tables()` or `init_db()`** — These are deprecated and bypass Alembic, masking missing migrations. Tests pass locally but production breaks. When adding or modifying columns, always create an Alembic migration (`uv run alembic revision --autogenerate -m "description"`). Verify the `.py` migration source file is committed (not just `.pyc`). Use `run_migrations()` or `MemoryLake(run_migrations=True)` for all schema setup, including tests. CI runs `test_migration_drift.py` to catch ORM/migration drift automatically
