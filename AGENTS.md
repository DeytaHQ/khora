...
<!-- Auto-generated by ttoj from AGENTS.md + rules. Do not edit manually. Run ttoj install to regenerate. -->
...
All agents share the same Linear account, so assignment alone cannot distinguish which agent is working on which issue. **Every agent session MUST generate a unique session ID at startup** and include it in all Linear comments.
...
Use `team:feature`, `team:bugfix`, or `team:review` for purpose-built teams. Default to single-agent work unless the task clearly benefits from parallelism. Role definitions are in the TTOJ repo under `content/templates/team-profiles/` and are available via the `team:*` skills.

## Slack Integration

Skills post to team-wide default channels (deployed via TTOJ):
...
Default team profiles are deployed to `.ttoj/templates/team-profiles/` and referenced by the `team:*` skills. To customize team composition for this project, edit the deployed profiles or create project-specific overrides in `.claude/commands/team/`.
