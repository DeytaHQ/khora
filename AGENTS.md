...
Use the `team-feature`, `team-bugfix`, or `team-review` skills for purpose-built teams. Default to single-agent work unless the task clearly benefits from parallelism.
...
Slack notifications are handled through skills: `slack-pr-notify`, `slack-merge-notify`, `slack-prd-notify`, `slack-notify`.
...
- **File operations**: Prefer dedicated agent tools (Read, Write, Edit, Grep, Glob) over shell equivalents.
