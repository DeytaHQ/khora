...
| `KHORA_QUERY_SHADOW_SCORING` | `false` | **Opt-in, observe-only (#1479).** Compute a candidate ranking alongside the live ranking on each recall and record the divergence (rank correlation, top-k overlap, biggest movers) under `RecallResult.engine_info["shadow_scoring"]`. The returned results are always the live ranking - shadow never reorders them. Genuinely zero-cost when off. |
