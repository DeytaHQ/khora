...
| `KHORA_STORAGE_GRAPH_POOL_KEEPALIVE_ENABLED` | `false` | Opt-in connection-pool keepalive. When `true`, Khora fires periodic `RETURN 1` pings on idle pooled connections so intermediaries (load balancers, firewalls) don't drop them before the driver's own liveness check fires. Zero-cost when `false`. |
| `KHORA_STORAGE_GRAPH_POOL_KEEPALIVE_INTERVAL_MS` | `15000` | Keepalive ping interval in ms (clamped 50–60000). Default 15000 ms is below the driver's 30 s liveness window so connections are exercised before they go stale. Only active when `KHORA_STORAGE_GRAPH_POOL_KEEPALIVE_ENABLED=true`. |
