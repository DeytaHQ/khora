...
config = KhoraConfig(
    ...
    query=QueryConfig(
        recency_decay_days=30,  # global default; 7 is a conversational-recency opt-in (#1421)
    ),
)
...
query:
  recency_decay_days: 30   # global default; set 7 only for conversational-recency workloads (#1421)
