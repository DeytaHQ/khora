...
class SurrealDBEventStoreAdapter:
    ...
    async def get_events(
        ...
        *,
        ...
        rows = await self._conn.query(
            f"SELECT * FROM {_TABLE} WHERE {where} ORDER BY timestamp DESC LIMIT $lim START $off",  # noqa: S608
            bindings,
        )
        return [self._row_to_event(r) for r in rows]
    ...
    async def count_events(
        ...
        *,
        ...
        row = await self._conn.query_one(
            f"SELECT count() AS total FROM {_TABLE} WHERE {where} GROUP ALL",  # noqa: S608
            bindings,
        )
        if row is None:
            return 0
