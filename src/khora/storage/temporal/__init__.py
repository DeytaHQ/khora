"""Temporal vector store: protocol, factory, and backend implementations.

Backends implement temporal-aware chunk storage:
...
- turbopuffer: serverless vector + BM25
- surrealdb: unified graph + vector + relational
- sqlite_lance: embedded aiosqlite + LanceDB

The neutral temporal data types (``TemporalChunk``, ``TemporalFilter``,
``TemporalSearchResult``, ``document_denorm_fields``, ``temporal_chunk_to_chunk``)
live in :mod:`khora.core.temporal` and are re-exported here for convenience.
"""
