...
_BUILTIN_PATTERNS: list[_Pattern] = [
    _Pattern(
        name="section_number",
        pattern=re.compile(
            r"(?:Section|Sec\.)\s*(\d+[A-Za-z]?\.\d+[A-Za-z]?\.?(?:\d+\.?)*)"
        ),
    ...
    _Pattern(
        name="ordinance_ref",
        pattern=re.compile(
            r"Ord\.?\s*No\.?\s*([\d,]+)\s*,\s*Eff\.?\s*(\d{1,2}/\d{1,2}/\d{2,4})"
        ),
                    ...
                    try:
                        patterns.append(
                            _Pattern(
                                ...
                                entity_type=rule.entity_types[0]
    ...
    def _extract_from_text(self, text: str) -> ExtractionResult:
        ...
        for pat in self._get_patterns():
            for match in pat.pattern.finditer(text):
                ...
                if (
                    self._expertise
                    and hasattr(self._expertise, "correlation_rules")
                ):
                    for rule in self._expertise.correlation_rules:
                        if (
                            rule.name == pat.name
