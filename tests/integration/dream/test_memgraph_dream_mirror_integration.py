"""Cross-store live-set invariant for the Memgraph flat soft-delete mirror (#1278).

Phase 4 of the dream-on-graph umbrella (#1282). Memgraph has no versioning
primitives and no APOC, so its dream graph mirror is FLAT soft-delete only: SET
``valid_until`` by id (no :EntityVersion snapshot), endpoint rewrite = re-create
+ delete, relabel = re-create + delete. This is the Memgraph analogue of the
Neo4j cross-store gate (``test_cross_store_live_set_invariant.py``, #1268/#1272).

The P1-4 master-gate property: after a dream apply + post-commit mirror on a real
pg+Memgraph stack, the graph-preferring ``list_relationships`` / ``list_entities``
read path and the PG ground-truth live set are byte-identical, and pruned /
merged self-loop edges plus soft-retired entities are invisible to graph recall.

Acceptance criteria (#1278): the invariant passes for **prune + dedupe
self-loop** on the docker-compose Memgraph stack.

How to run locally::

    docker compose up -d postgres memgraph   # postgres (5434) + memgraph (7689)
    KHORA_DATABASE_URL=postgresql+asyncpg://khora:khora@localhost:5434/khora \\
    KHORA_MEMGRAPH_URL=bolt://localhost:7689 \\
        uv run pytest tests/integration/dream/test_memgraph_dream_mirror_integration.py -v
"""
...
from khora.config.schema import KhoraConfig, MemgraphConfig
...
MEMGRAPH_URL = os.environ.get("KHORA_MEMGRAPH_URL", "bolt://localhost:7689")
MEMGRAPH_USER = os.environ.get("KHORA_MEMGRAPH_USERNAME", "memgraph")
MEMGRAPH_PASSWORD = os.environ.get("KHORA_MEMGRAPH_PASSWORD", "")
...
pytestmark = [
    ...
    pytest.mark.skipif(
        not (_reachable(DATABASE_URL, 5432) and _reachable(MEMGRAPH_URL, 7687)),
        reason="pg+memgraph not reachable (run `docker compose up -d postgres memgraph`)",
    ),
]
...
async def kb() -> AsyncIterator[Khora]:
    ...
    # Null out any legacy Neo4j URL the shell env (KHORA_NEO4J_URL) may have
    # leaked into the config, so the graph backend is unambiguously Memgraph.
    config.neo4j_url = None
    config.storage.neo4j_url = None
    config.storage.graph = MemgraphConfig(
        url=MEMGRAPH_URL,
        user=MEMGRAPH_USER,
    ...
    await kb.connect()
    # The shared compose Memgraph instance persists across runs; clear any nodes
    # left by an earlier test so list_* baselines are clean per-namespace.
    # (Each test still uses a fresh namespace id, so this is belt-and-braces.)
    try:
        yield kb
    finally:
        await kb.disconnect()
...
# --- Seed + live-set helpers (mirrors test_cross_store_live_set_invariant.py) ---


async def _seed_entity_both(kb: Khora, ns_row_id: UUID, name: str) -> UUID:
    """Seed one entity into PG (entities table) and Memgraph with a matching id."""
...
# ---------------------------------------------------------------------------
# The master gate: prune + dedupe self-loop (the #1278 acceptance criteria)
# ---------------------------------------------------------------------------
...
async def test_dedupe_self_loop_converges_and_replay_is_idempotent(kb: Khora) -> None:
    """dedupe self-loop: absorbed entity flat-retired + self-loop invalidated in
    both stores; a mirror replay (same undo) leaves the live sets unchanged."""
    ns = await kb.create_namespace()
...
async def test_normalize_schema_relabel_is_documented_skip_not_silent_divergence(kb: Khora) -> None:
    """normalize_schema: its graph-label relabel is NOT mirrored (deferred, same as
    Neo4j). The live id sets stay byte-identical (a relabel removes no rows), and
    the mirror surfaces a STRUCTURED skip on the result, not a silent divergence."""
    ...
    # normalize_schema is excluded from MIRRORABLE_OP_KINDS, so even though the
    # backend advertises it, the mirror surfaces a documented skip (ADR-001).
