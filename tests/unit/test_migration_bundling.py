...
class TestMigrationPackageStructure:
    ...
    @pytest.mark.unit
    def test_versions_dir_has_49_files(self):
        """versions/ directory contains 49 migration files."""
        ...
        assert len(migration_files) == 49, (
            f"Expected 49 migration files, found {len(migration_files)}: {[f.name for f in migration_files]}"
        )
