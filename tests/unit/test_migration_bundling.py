...
class TestMigrationPackageStructure:
    ...
    @pytest.mark.unit
    def test_versions_dir_has_51_files(self):
        """versions/ directory contains 51 migration files."""
        ...
        assert len(migration_files) == 51, (
            f"Expected 51 migration files, found {len(migration_files)}: {[f.name for f in migration_files]}"
        )
