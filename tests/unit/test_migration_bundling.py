...
class TestMigrationPackageStructure:
    ...
    @pytest.mark.unit
    def test_versions_dir_has_48_files(self):
        """versions/ directory contains 48 migration files."""
        ...
        assert len(migration_files) == 48, (
            f"Expected 48 migration files, found {len(migration_files)}: {[f.name for f in migration_files]}"
        )
