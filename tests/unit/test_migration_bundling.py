...
class TestMigrationPackageStructure:
    ...
    @pytest.mark.unit
    def test_versions_dir_has_50_files(self):
        """versions/ directory contains 50 migration files."""
        ...
        assert len(migration_files) == 50, (
            f"Expected 50 migration files, found {len(migration_files)}: {[f.name for f in migration_files]}"
        )
