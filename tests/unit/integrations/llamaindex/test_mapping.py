...
def _mk_chunk(*, content: str = "hello world", score: float = 0.87) -> RecallChunk:
    return RecallChunk(
        id=uuid4(),
        document_id=uuid4(),
        content=content,
...
def _mk_entity(name: str = "Alice", entity_type: str = "PERSON") -> RecallEntity:
    return RecallEntity(
        id=uuid4(),
        name=name,
...
def test_chunk_to_node_with_score_basic():
    ...
    chunk = _mk_chunk(content="hello", score=0.87)
    namespace_id = uuid4()
    ...
    assert node.node.metadata["namespace_id"] == str(namespace_id)
