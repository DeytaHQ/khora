...
DYT-4607 widened the builder's signature to take a ``chunks: list[Chunk]``
argument so each emitted ``ASSOCIATED_WITH`` edge can carry
``source_chunk_ids`` / ``source_document_ids`` provenance back to the
chunk (and document) it was synthesized from.
