...
class TestBackendSessionParam:
    ...
    async def test_postgresql_create_document_with_session(self):
        ...
        mock_doc.metadata.source = None
        mock_doc.metadata.source_type = None
        mock_doc.metadata.content_type = "text/plain"
        mock_doc.metadata.title = "test"
        mock_doc.metadata.author = None
        mock_doc.metadata.language = None
        mock_doc.metadata.checksum = "abc"
        mock_doc.metadata.size_bytes = 4
        mock_doc.metadata.custom = {}
    ...
    async def test_pgvector_create_chunks_batch_with_session(self):
        ...
        mock_chunk.metadata.end_char = 4
        mock_chunk.metadata.token_count = 1
        mock_chunk.metadata.custom = {}
